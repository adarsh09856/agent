...
export function CreateCampaignDialog({ open, onOpenChange }: CreateCampaignDialogProps) {
  ...
  const handleNext = () => {
    ...
    if (step === 2) {
      ...
      } else if (selectedAgent?.telephonyProvider === 'custom-voice-engine') {
        // Custom Voice Engine can use any phone number or none (it's optional)
        // No strict phone validation required
      } else {
