...
export default function CreditPackages() {
  ...
  const createPackage = useMutation({
    ...
    onSuccess: () => {
      ...
      toast({ title: t("adminDashboard.creditPackages.packageCreated") });
      setShowCreateDialog(false);
      resetForm();
    },
    onError: (error: any) => {
      toast({
        title: t("adminDashboard.creditPackages.createFailed"),
  ...
  const updatePackage = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/api/admin/credit-packages"] });
      toast({ title: t("adminDashboard.creditPackages.packageUpdated") });
      setEditingPackage(null);
      resetForm();
    },
    onError: (error: any) => {
      toast({
        title: t("adminDashboard.creditPackages.updateFailed"),
        description: error.message,
        variant: "destructive"
      });
    }
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold">{t("adminDashboard.creditPackages.title")}</h2>
          <p className="text-muted-foreground">
            {t("adminDashboard.creditPackages.description")}
          </p>
        </div>
        <Button onClick={() => setShowCreateDialog(true)} data-testid="button-create-package">
          <Plus className="mr-2 h-4 w-4" />
          {t("adminDashboard.creditPackages.createPackage")}
        </Button>
      </div>
              ...
              <CardContent>
                <div className="space-y-2">
                  <div className="flex justify-between items-center">
                    <span className="text-2xl font-bold">{pkg.credits} {t("adminDashboard.creditPackages.credits")}</span>
                  </div>
                  ...
                  {paymentGateway?.stripeEnabled && (
                    <div className="text-sm text-muted-foreground border-t pt-2 mt-2">
                      {stripeCurrencySymbol}{(pkg.price / pkg.credits).toFixed(3)} {t("adminDashboard.creditPackages.perCredit")}
                    </div>
                  )}
                  <Button
                  ...
                  >
                    {pkg.isActive ? t("adminDashboard.creditPackages.deactivate") : t("adminDashboard.creditPackages.activate")}
                  </Button>
                </div>
              </CardContent>
            </Card>
      ...
      <Dialog open={showCreateDialog || !!editingPackage} 
        ...
        <DialogContent>
          <DialogHeader>
            <DialogTitle>
              {editingPackage ? t("adminDashboard.creditPackages.editPackage") : t("adminDashboard.creditPackages.createTitle")}
            </DialogTitle>
            <DialogDescription>
              {editingPackage 
                ? t("adminDashboard.creditPackages.editDescription")
                : t("adminDashboard.creditPackages.createDescription")}
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-4">
            <div>
              <div className="flex items-center">
                <Label>{t("adminDashboard.creditPackages.packageName")}</Label>
                <InfoTooltip content={t("adminDashboard.creditPackages.packageNameTooltip")} />
              </div>
              <Input
                value={formData.name}
                onChange={(e) => setFormData({ ...formData, name: e.target.value })}
                placeholder={t("adminDashboard.creditPackages.packageNamePlaceholder")}
                data-testid="input-package-name"
              />
            </div>
            <div>
              <div className="flex items-center">
                <Label>{t("adminDashboard.creditPackages.packageDescLabel")}</Label>
                <InfoTooltip content={t("adminDashboard.creditPackages.packageDescTooltip")} />
              </div>
              <Input
                value={formData.description}
                onChange={(e) => setFormData({ ...formData, description: e.target.value })}
                placeholder={t("adminDashboard.creditPackages.packageDescPlaceholder")}
                data-testid="input-package-description"
              />
            </div>
            <div>
              <div className="flex items-center">
                <Label>{t("adminDashboard.creditPackages.creditsLabel")}</Label>
                <InfoTooltip content={t("adminDashboard.creditPackages.creditsTooltip")} />
              </div>
              <Input
                type="number"
            ...
            {formData.credits > 0 && (
              <div className="text-sm text-muted-foreground">
                {t("adminDashboard.creditPackages.pricePerCredit", { price: (formData.price / formData.credits).toFixed(3) })}
              </div>
            )}
          </div>
          <DialogFooter>
            ...
            >
              {(createPackage.isPending || updatePackage.isPending) ? (
                ...
                editingPackage ? t("adminDashboard.creditPackages.updatePackage") : t("adminDashboard.creditPackages.createPackage")
              )}
            </Button>
          </DialogFooter>
        </DialogContent>
