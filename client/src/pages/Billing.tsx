...
import { CreditCard, Download, Plus, Loader2, Check, Crown, Calendar, AlertCircle, Wallet, Coins, Receipt, TrendingUp, Sparkles, ArrowUpRight, Clock, Globe, FileText, ChevronLeft, ChevronRight, Video, Play } from "lucide-react";
  ...
  return (
    <div className="space-y-8">
      <div className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-slate-50 via-slate-100/50 to-indigo-50 dark:from-slate-900/80 dark:via-slate-800/50 dark:to-indigo-950/40 border border-slate-200 dark:border-slate-700/50 p-6 md:p-8">
        ...
        <div className="relative mt-6 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-4">
          ...
          <div className="bg-white/80 dark:bg-slate-800/60 backdrop-blur-sm rounded-xl p-4 border border-slate-200/50 dark:border-slate-700/50">
            <div className="flex items-center gap-2">
              ...
              <div className="text-2xl font-bold text-blue-700 dark:text-blue-300">{(user?.subscriptionMinutes ?? 0).toLocaleString()} <span className="text-sm font-normal">min</span></div>
            </div>
            <div className="text-blue-600/70 dark:text-blue-400/70 text-sm">Monthly Plan Minutes</div>
          </div>
          <div className="bg-white/80 dark:bg-slate-800/60 backdrop-blur-sm rounded-xl p-4 border border-slate-200/50 dark:border-slate-700/50">
            <div className="flex items-center gap-2">
      ...
      {/* Platform Metering & BYOK Governance Summary */}
      <div className="bg-gradient-to-r from-blue-50/60 via-indigo-50/50 to-purple-50/40 dark:from-slate-900/60 dark:via-indigo-950/30 dark:to-purple-950/20 border border-indigo-100/80 dark:border-indigo-900/50 rounded-xl p-4 flex flex-col md:flex-row md:items-center justify-between gap-4 shadow-sm">
        <div className="flex items-start gap-3">
          <Clock className="h-5 w-5 text-indigo-600 dark:text-indigo-400 mt-0.5 shrink-0" />
          <div className="space-y-0.5">
            <h4 className="text-sm font-semibold text-foreground">Dual-Tier Credit & BYOK Metering Policy</h4>
            <p className="text-xs text-muted-foreground leading-relaxed">
              Voice calls automatically consume your included <span className="font-semibold text-foreground">Monthly Plan Minutes</span> first. When exhausted, calls draw from your <span className="font-semibold text-foreground">Credit Wallet</span>. If Bring-Your-Own-Key (BYOK) is active and configured for your account under Settings, calls bypass platform credit deduction completely ($0 charge).
            </p>
          </div>
        </div>
      </div>
