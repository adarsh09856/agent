    ...
    <div className="space-y-6">
      ...
      <Tabs defaultValue="overview" className="space-y-4">
        ...
        <TabsContent value="overview" className="space-y-4">
          ...
          {(campaign.batchJobId || batchJobData?.batchJob || (campaign.totalContacts && campaign.totalContacts > 0) || calls.length > 0) && (
            <Card className="p-6" data-testid="card-batch-job-status">
              ...
              {batchLoading ? (
                ...
                <div className="space-y-4">
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                    ...
                    <div>
                      ...
                      <p className="font-mono text-xs" data-testid="text-batch-job-id">
                        {(campaign.batchJobId || batchJobData?.batchJob?.id || campaign.id)?.slice(0, 12)}...
                      </p>
                    </div>
                    <div>
                  ...
                  {batchJobData.stats && (() => {
                    const calculatedProgress = batchJobData.stats.progress ?? (
                      batchJobData.batchJob?.total_calls_scheduled && batchJobData.batchJob.total_calls_scheduled > 0
                        ? Math.min(100, Math.round(((batchJobData.batchJob.total_calls_dispatched || 0) / batchJobData.batchJob.total_calls_scheduled) * 100))
                        : (batchJobData.batchJob?.status === 'completed' ? 100 : 0)
                    );
                    return (
                      <>
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-5 gap-3 text-sm">
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            <span className="font-medium" data-testid="text-batch-scheduled">
                              {batchJobData.stats.scheduled ?? 0}
                            </span>
                          </div>
                          <div className="flex items-center gap-2">
                            ...
                            <span className="font-medium" data-testid="text-batch-in-progress">
                              {batchJobData.stats.in_progress ?? 0}
                            </span>
                          </div>
                          <div className="flex items-center gap-2">
                            ...
                            <span className="font-medium" data-testid="text-batch-completed">
                              {batchJobData.stats.completed ?? 0}
                            </span>
                          </div>
                          <div className="flex items-center gap-2">
