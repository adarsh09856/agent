...
export default function Dashboard() {
  ...
  return (
    <div className="space-y-4 md:space-y-6">
      ...
      {/* System Performance & KPIs (Enhanced Data View) */}
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <Card className="bg-card shadow-sm border border-border/50 hover-elevate transition-all">
          <CardContent className="p-5 flex flex-col justify-between h-full">
            <div>
              <div className="flex items-center justify-between mb-3">
                ...
                <span className="text-sm font-bold text-emerald-600 dark:text-emerald-400">
                  {Math.round(
                    ((dashboard?.callTypeStats?.incoming?.successRate || 0) + 
                     (dashboard?.callTypeStats?.outgoing?.successRate || 0)) / 2
                  )}%
                </span>
              </div>
              <div className="w-full bg-muted rounded-full h-2">
                <div 
                  ...
                  style={{ 
                    width: `${((dashboard?.callTypeStats?.incoming?.successRate || 0) + 
                             (dashboard?.callTypeStats?.outgoing?.successRate || 0)) / 2}%` 
                  }}
                />
              </div>
            ...
            <p className="text-xs text-muted-foreground mt-3">
              Average success rate of calls processed by active agents this week.
            </p>
          </CardContent>
        </Card>
        ...
        <Card className="bg-card shadow-sm border border-border/50 hover-elevate transition-all">
          <CardContent className="p-5 flex flex-col justify-between h-full">
            <div>
              <div className="flex items-center justify-between mb-3">
                <span className="text-sm font-medium text-muted-foreground">Talk Time Efficiency</span>
                <span className="text-sm font-bold text-blue-600 dark:text-blue-400">
                  {formatDuration(
                    Math.round(
                      ((dashboard?.callTypeStats?.incoming?.avgDuration || 0) + 
                       (dashboard?.callTypeStats?.outgoing?.avgDuration || 0)) / 2
                    )
                  )} avg
                </span>
              </div>
            ...
            <p className="text-xs text-muted-foreground mt-3">
              Average call handling duration for both incoming and outgoing connections.
            </p>
          </CardContent>
        </Card>
        ...
        <Card className="bg-card shadow-sm border border-border/50 hover-elevate transition-all">
          <CardContent className="p-5 flex flex-col justify-between h-full">
            <div>
              <div className="flex items-center justify-between mb-3">
                ...
                <span className="text-sm font-bold text-purple-600 dark:text-purple-400">
                  {dashboard?.callTypeStats?.campaign?.active || 0} Active
                </span>
              </div>
              <div className="w-full bg-muted rounded-full h-2">
                <div 
                  ...
                  style={{ 
                    width: `${dashboard?.callTypeStats?.campaign?.count 
                      ? ((dashboard?.callTypeStats?.campaign?.active || 0) / dashboard.callTypeStats.campaign.count) * 100 
                      : 0}%` 
                  }}
                />
              </div>
            ...
            <p className="text-xs text-muted-foreground mt-3">
              Ratio of actively running marketing/outbound campaigns compared to total campaigns.
            </p>
          </CardContent>
        </Card>
      </div>
