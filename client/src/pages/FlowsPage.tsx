...
import { Loader2, Plus, Edit, Trash2, GitBranch, Play, FileText, LayoutTemplate, AlertTriangle, Video } from "lucide-react";
...
export default function FlowsPage() {
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: () => {
      toast({
        title: t('flows.flowDeleted'),
        description: t('flows.flowDeletedDescription'),
      });
  ...
  const toggleActiveMutation = useMutation({
    ...
    onSuccess: () => {
      toast({
        title: t('flows.flowUpdated'),
        description: t('flows.flowUpdatedDescription'),
      });
      queryClient.invalidateQueries({ queryKey: ["/api/flow-automation/flows"] });
      queryClient.invalidateQueries({ queryKey: ["/api/usage"] });
    },
  ...
  const createTemplateFlowMutation = useMutation({
    ...
    onSuccess: (data: any) => {
      toast({
        title: t('flows.templateCreated'),
        description: t('flows.templateCreatedDescription'),
      });
      queryClient.invalidateQueries({ queryKey: ["/api/flow-automation/flows"] });
      queryClient.invalidateQueries({ queryKey: ["/api/usage"] });
      if (data?.id) {
    ...
    onError: (error: any) => {
      toast({
        title: t('flows.errorCreatingTemplate'),
        description: error.message,
        variant: "destructive",
      });
    },
  ...
  return (
    <div className="p-4 md:p-8 space-y-4 md:space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl md:text-3xl font-bold" data-testid="text-page-title">{t('flows.title')}</h1>
          ...
          <div className="flex items-center gap-3 flex-wrap">
            <TabsList data-testid="tabs-flows">
              <TabsTrigger value="flows" data-testid="tab-flows">
                ...
                {t('flows.myFlows', { count: flows?.length || 0 })}
              </TabsTrigger>
            ...
            {activeTab === "flows" && flowUsage && flowUsage.max !== 999 && flowUsage.max !== -1 && (
              <div className="flex flex-col gap-0.5" data-testid="text-flow-usage">
                ...
                >
                  ...
                  <span>{t('flows.flowsCount', { current: flowUsage.current, max: flowUsage.max })}</span>
                </div>
                {isAtLimit && (
            ...
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                ...
                >
                  ...
                  {createNewFlowMutation.isPending || createTemplateFlowMutation.isPending ? t('flows.creating') : t('flows.createFlow')}
                </Button>
              </DropdownMenuTrigger>
      ...
      {!flows || flows.length === 0 ? (
        <Card>
          <CardContent className="flex flex-col items-center justify-center py-16">
            ...
            <h3 className="text-xl font-semibold mb-2">{t('flows.noFlowsYet')}</h3>
            ...
            <div className="space-y-3">
              ...
              >
                <Plus className="w-4 h-4 mr-2" />
                {createNewFlowMutation.isPending ? t('flows.creating') : t('flows.createBlankFlow')}
              </Button>
              <Button 
              ...
              >
                <FileText className="w-4 h-4 mr-2" />
                {createTemplateFlowMutation.isPending ? t('flows.creating') : t('flows.useCallTransferTemplate')}
              </Button>
            </div>
          </CardContent>
        </Card>
                ...
                <CardContent className="space-y-4">
                  <div className="flex items-center gap-2 flex-wrap">
                    <Badge variant="outline" className="text-xs">
                      {flow.nodes?.length || 0} {t('flows.nodes')}
                    </Badge>
                    <Badge variant="outline" className="text-xs">
                      {flow.edges?.length || 0} {t('flows.connections')}
                    </Badge>
                  </div>
                  <div className="text-xs text-muted-foreground">
                    {t('flows.lastUpdated')}: {new Date(flow.updatedAt).toLocaleDateString()}
                  </div>
                </CardContent>
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              {t('flows.deleteConfirmation', { name: flowToDelete?.name })}
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            ...
            >
              {deleteMutation.isPending ? t('common.deleting') : t('flows.deleteFlow')}
            </AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
