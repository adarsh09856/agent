...
type GatewayType = 'stripe' | 'razorpay' | 'paypal' | 'paystack' | 'mercadopago' | 'yookassa';
...
interface Plan {
  ...
  yookassaMonthlyPrice: string | null;
  yookassaYearlyPrice: string | null;
...
export function UpgradePlansContent() {
  ...
  const availableCurrencies = useMemo((): CurrencyOption[] => {
    ...
    if (paymentGateway.yookassaEnabled) {
      const curr = (paymentGateway.yookassaCurrency || 'RUB').toUpperCase();
      currencyMap.set(curr, [...(currencyMap.get(curr) || []), 'yookassa']);
    }
  ...
  const getPlanSyncedGatewaysForCurrency = (plan: Plan, currencyCode: string, period: "monthly" | "yearly"): GatewayType[] => {
    ...
    const allPricesZero = [
      ...
      plan.yookassaMonthlyPrice, plan.yookassaYearlyPrice,
    ...
    return gateways.filter((gateway) => {
      switch (gateway) {
        ...
        case 'yookassa':
          return period === 'monthly' ? !!plan.yookassaMonthlyPlanId : !!plan.yookassaYearlyPlanId;
        default:
          return false;
      }
    });
  ...
  const getGatewayInfo = (gateway: GatewayType): { icon: React.ComponentType<{ className?: string }>; name: string } => {
    const info: Record<GatewayType, { icon: React.ComponentType<{ className?: string }>; name: string }> = {
      ...
      yookassa: { icon: CreditCard, name: 'YooKassa' },
    };
    return info[gateway];
  };
  ...
  const stripeCheckout = useMutation({
    mutationFn: async ({ planId, billingPeriod }: { planId: string; billingPeriod: string }) => {
      ...
      if (!response.ok) {
        ...
        throw new Error(error.error || t('upgradePage.subscriptionFailed', 'Failed to create checkout session'));
      }
    ...
    onError: (error: any) => {
      toast({
        title: t('upgradePage.checkoutFailed', 'Checkout Failed'),
  ...
  const razorpaySubscription = useMutation({
    mutationFn: async ({ planId, billingPeriod }: { planId: string; billingPeriod: string }) => {
      ...
      if (!response.ok) {
        const error = await response.json();
        throw new Error(error.error || t('upgradePage.subscriptionFailed', 'Failed to create subscription'));
      }
      return response.json();
    },
    onSuccess: (data) => {
      if (!window.Razorpay || !razorpayLoaded) {
        toast({
          title: t('upgradePage.razorpayNotLoaded', 'Razorpay Not Loaded'),
          description: t('upgradePage.pleaseWait', 'Please wait a moment and try again'),
          variant: "destructive",
        });
        setProcessingGateway(null);
        return;
        ...
        handler: async function (response: any) {
          try {
            ...
            if (verifyResponse.ok) {
              toast({
                title: t('upgradePage.subscriptionActive', 'Subscription Active'),
                description: t('upgradePage.subscriptionSuccess', 'Your subscription has been activated successfully!'),
              });
              window.location.href = "/app/billing?success=true";
            } else {
              const error = await verifyResponse.json();
              throw new Error(error.error || t('upgradePage.verificationFailed', 'Verification failed'));
            }
          } catch (error: any) {
            toast({
              title: t('upgradePage.verificationFailed', 'Verification Failed'),
              description: error.message,
              variant: "destructive",
            });
          }
    ...
    onError: (error: any) => {
      toast({
        title: t('upgradePage.subscriptionFailed', 'Subscription Failed'),
        description: error.message,
        variant: "destructive",
      });
      setProcessingGateway(null);
  ...
  const paypalSubscription = useMutation({
    mutationFn: async ({ planId, billingPeriod }: { planId: string; billingPeriod: string }) => {
      ...
      if (!response.ok) {
        const error = await response.json();
        throw new Error(error.error || t('upgradePage.subscriptionFailed', 'Failed to create PayPal subscription'));
      }
      return response.json();
    },
    onSuccess: (data) => {
    ...
    onError: (error: any) => {
      toast({ title: t('upgradePage.subscriptionFailed', 'Subscription Failed'), description: error.message, variant: "destructive" });
      setProcessingGateway(null);
    },
  });
  ...
  const paystackSubscription = useMutation({
    mutationFn: async ({ planId, billingPeriod }: { planId: string; billingPeriod: string }) => {
      ...
      if (!response.ok) {
        const error = await response.json();
        throw new Error(error.error || t('upgradePage.subscriptionFailed', 'Failed to create Paystack subscription'));
      }
      return response.json();
    },
    onSuccess: (data) => {
  ...
  const mercadopagoSubscription = useMutation({
    mutationFn: async ({ planId, billingPeriod }: { planId: string; billingPeriod: string }) => {
      ...
      if (!response.ok) {
        const error = await response.json();
        throw new Error(error.error || t('upgradePage.subscriptionFailed', 'Failed to create MercadoPago subscription'));
      }
      return response.json();
    },
    onSuccess: (data) => {
  ...
  const yookassaSubscription = useMutation({
    mutationFn: async ({ planId, billingPeriod }: { planId: string; billingPeriod: string }) => {
      const response = await apiRequest("POST", "/api/yookassa/create-checkout-session", { planId, billingPeriod });
      if (!response.ok) {
        const error = await response.json();
        throw new Error(error.error || t('upgradePage.subscriptionFailed', 'Failed to create YooKassa checkout session'));
      }
      return response.json();
    },
    onSuccess: (data) => {
  ...
  const handleProceedToPayment = () => {
    ...
    switch (selectedGateway) {
      ...
      case 'razorpay':
        if (!razorpayLoaded) {
          toast({ title: t('upgradePage.paymentGatewayLoading', 'Payment Gateway Loading'), description: t('upgradePage.razorpayLoadingDesc', 'Razorpay is still loading. Please try again.'), variant: "destructive" });
          setProcessingGateway(null);
          return;
        }
      ...
      case 'mercadopago':
        ...
        break;
      case 'yookassa':
        yookassaSubscription.mutate({ planId: selectedPlan.id, billingPeriod });
        break;
    }
  };
            ...
            <div>
              <div className="flex items-center gap-3">
                <h1 className="text-2xl md:text-3xl font-bold text-foreground">
                  {isPremium ? t('upgradePage.yourPlan', 'Your Plan') : t('upgradePage.title', 'Upgrade Your Plan')}
                </h1>
                {isPremium && (
                  <Badge className="bg-indigo-600 hover:bg-indigo-700 text-white" data-testid="badge-premium-member">
                    <Crown className="h-3 w-3 mr-1" />
                    {t('upgradePage.premium', 'Premium')}
                  </Badge>
                )}
              </div>
              <p className="text-muted-foreground mt-0.5">
                {isPremium ? t('upgradePage.manageSubscription', 'Manage your subscription and billing') : t('upgradePage.choosePlan', 'Choose the plan that fits your needs')}
              </p>
            </div>
          </div>
            ...
            <div className="flex items-center gap-2">
              ...
              <Select value={effectiveCurrency} onValueChange={setSelectedCurrency}>
                <SelectTrigger className="w-[140px] bg-white/80 dark:bg-slate-800/60" data-testid="select-currency">
                  <SelectValue placeholder={t('upgradePage.currency', 'Currency')} />
                </SelectTrigger>
                <SelectContent>
            ...
            <div className="flex items-center justify-between flex-wrap gap-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="font-semibold text-foreground">
                    {t('upgradePage.currentPlan', { name: currentPlan.displayName, defaultValue: `Current: ${currentPlan.displayName}` })}
                  </h3>
              ...
              <div className="text-right">
                <div className="text-xl font-bold text-foreground">
                  {(() => {
                    if (currentPlan.name === "free") return t('upgradePage.free', 'Free');
                ...
                {currentPlan.name !== "free" && (
                  <p className="text-xs text-muted-foreground">{t('upgradePage.perMonth', 'per month')}</p>
                )}
              </div>
            </div>
          </div>
            ...
            >
              {isRecommended && !isCurrentPlan && (
                <div className="absolute top-0 left-0 right-0 bg-gradient-to-r from-slate-700 to-indigo-700 text-white text-xs font-medium py-1.5 text-center">
                  ...
                  {t('upgradePage.mostPopular', 'Most Popular')}
                </div>
              )}
              ...
              <div className={`p-6 space-y-6 ${isRecommended && !isCurrentPlan ? "pt-10" : ""}`}>
                ...
                <div className="pb-4 border-b border-slate-100 dark:border-slate-800">
                  {isFree ? (
                    <>
                      <div className="text-3xl font-bold text-foreground">{t('upgradePage.free', 'Free')}</div>
                      <p className="text-sm text-muted-foreground">{t('upgradePage.forever', 'Forever')}</p>
                    </>
                  ) : (
                    <>
                      <div className="flex items-baseline gap-1">
                        <span className="text-3xl font-bold text-foreground">{monthlyPrice}</span>
                        <span className="text-muted-foreground text-sm">/{t('upgradePage.month', 'month')}</span>
                      </div>
                      {yearlyPrice && yearlySavings && (
                        <p className="text-sm text-emerald-600 dark:text-emerald-400 mt-1">
                          {t('upgradePage.savePerYear', { symbol: monthlyPriceInfo.symbol, amount: yearlySavings, defaultValue: `Save ${monthlyPriceInfo.symbol}${yearlySavings}/year` })}
                        </p>
                      )}
                    </>
                  )}
                ...
                <div className="space-y-3">
                  {plan.maxAgents === -1 ? (
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-sm font-medium">{t('upgradePage.unlimitedAgents', 'Unlimited AI Agents')}</span>
                    </div>
                  ) : (
                    <div className="flex items-center gap-2">
                      <Check className="h-4 w-4 text-emerald-600 dark:text-emerald-400 flex-shrink-0" />
                      <span className="text-sm">
                        {t('upgradePage.agentsCount', { count: plan.maxAgents, defaultValue: `${plan.maxAgents} AI Agent${plan.maxAgents > 1 ? "s" : ""}` })}
                      </span>
                    </div>
                  )}
                  ...
                  {plan.maxCampaigns === -1 ? (
                    <div className="flex items-center gap-2">
                      <Check className="h-4 w-4 text-emerald-600 dark:text-emerald-400 flex-shrink-0" />
                      <span className="text-sm font-medium">{t('upgradePage.unlimitedCampaigns', 'Unlimited Campaigns')}</span>
                    </div>
                  ) : (
                    <div className="flex items-center gap-2">
                      <Check className="h-4 w-4 text-emerald-600 dark:text-emerald-400 flex-shrink-0" />
                      <span className="text-sm">
                        {t('upgradePage.campaignsCount', { count: plan.maxCampaigns, defaultValue: `${plan.maxCampaigns} Campaign${plan.maxCampaigns > 1 ? "s" : ""}` })}
                      </span>
                    </div>
                  )}
                  ...
                  {plan.maxContactsPerCampaign === -1 ? (
                    <div className="flex items-center gap-2">
                      <Check className="h-4 w-4 text-emerald-600 dark:text-emerald-400 flex-shrink-0" />
                      <span className="text-sm font-medium">{t('upgradePage.unlimitedContacts', 'Unlimited Contacts')}</span>
                    </div>
                  ) : (
                    <div className="flex items-center gap-2">
                      <Check className="h-4 w-4 text-emerald-600 dark:text-emerald-400 flex-shrink-0" />
                      <span className="text-sm">
                        {t('upgradePage.maxContacts', { count: plan.maxContactsPerCampaign, defaultValue: `Max ${plan.maxContactsPerCampaign} contacts` })}
                      </span>
                    </div>
                  )}
                  ...
                  {plan.canPurchaseNumbers && (
                    <div className="flex items-center gap-2">
                      <Check className="h-4 w-4 text-emerald-600 dark:text-emerald-400 flex-shrink-0" />
                      <span className="text-sm">{t('upgradePage.ownPhoneNumbers', 'Own phone numbers')}</span>
                    </div>
                  )}
                  ...
                  {plan.canChooseLlm && (
                    <div className="flex items-center gap-2">
                      <Check className="h-4 w-4 text-emerald-600 dark:text-emerald-400 flex-shrink-0" />
                      <span className="text-sm">{t('upgradePage.chooseLlm', 'Choose your LLM')}</span>
                    </div>
                  )}
                  ...
                  {plan.maxFlows !== undefined && plan.maxFlows > 0 && (
                    <div className="flex items-center gap-2">
                      <Check className="h-4 w-4 text-emerald-600 dark:text-emerald-400 flex-shrink-0" />
                      <span className="text-sm">
                        {plan.maxFlows >= 999 ? t('upgradePage.unlimitedFlows', 'Unlimited Flow Automations') : t('upgradePage.flowsCount', { count: plan.maxFlows, defaultValue: `${plan.maxFlows} Flow Automation${plan.maxFlows !== 1 ? "s" : ""}` })}
                      </span>
                    </div>
                  )}
                  ...
                  {plan.maxKnowledgeBases !== undefined && plan.maxKnowledgeBases > 0 && (
                    <div className="flex items-center gap-2">
                      <Check className="h-4 w-4 text-emerald-600 dark:text-emerald-400 flex-shrink-0" />
                      <span className="text-sm">
                        {plan.maxKnowledgeBases >= 999 ? t('upgradePage.unlimitedKnowledgeBases', 'Unlimited Knowledge Bases') : t('upgradePage.knowledgeBasesCount', { count: plan.maxKnowledgeBases, defaultValue: `${plan.maxKnowledgeBases} Knowledge Base${plan.maxKnowledgeBases !== 1 ? "s" : ""}` })}
                      </span>
                    </div>
                  )}
                  ...
                  {plan.maxWebhooks !== undefined && plan.maxWebhooks > 0 && (
                    <div className="flex items-center gap-2">
                      <Check className="h-4 w-4 text-emerald-600 dark:text-emerald-400 flex-shrink-0" />
                      <span className="text-sm">
                        {plan.maxWebhooks >= 999 ? t('upgradePage.unlimitedWebhooks', 'Unlimited Webhooks') : t('upgradePage.webhooksCount', { count: plan.maxWebhooks, defaultValue: `${plan.maxWebhooks} Webhook${plan.maxWebhooks !== 1 ? "s" : ""}` })}
                      </span>
                    </div>
                  )}
                  ...
                  {plan.maxPhoneNumbers !== undefined && plan.maxPhoneNumbers > 0 && (
                    <div className="flex items-center gap-2">
                      <Check className="h-4 w-4 text-emerald-600 dark:text-emerald-400 flex-shrink-0" />
                      <span className="text-sm">
                        {plan.maxPhoneNumbers >= 999 ? t('upgradePage.unlimitedPhoneNumbers', 'Unlimited Phone Numbers') : t('upgradePage.phoneNumbersCount', { count: plan.maxPhoneNumbers, defaultValue: `${plan.maxPhoneNumbers} Phone Number${plan.maxPhoneNumbers !== 1 ? "s" : ""}` })}
                      </span>
                    </div>
                  )}
                  ...
                  {plan.includedCredits > 0 && (
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-sm font-medium">
                        {t('upgradePage.includedCredits', { count: plan.includedCredits, defaultValue: `${plan.includedCredits} included credits` })}
                      </span>
                    </div>
                  )}
                  ...
                  {!isFree && (
                    <div className="flex items-center gap-2">
                      <Check className="h-4 w-4 text-emerald-600 dark:text-emerald-400 flex-shrink-0" />
                      <span className="text-sm">{t('upgradePage.prioritySupport', 'Priority support')}</span>
                    </div>
                  )}
                  ...
                  {sipPluginEnabled && plan.sipEnabled && !isFree && (
                    <div className="flex items-center gap-2">
                      <Check className="h-4 w-4 text-emerald-600 dark:text-emerald-400 flex-shrink-0" />
                      <span className="text-sm">{t('upgradePage.sipTrunkAccess', 'SIP Trunk Access')}</span>
                    </div>
                  )}
                  ...
                  {restApiPluginEnabled && plan.restApiEnabled && !isFree && (
                    <div className="flex items-center gap-2">
                      <Check className="h-4 w-4 text-emerald-600 dark:text-emerald-400 flex-shrink-0" />
                      <span className="text-sm">{t('upgradePage.restApiAccess', 'REST API Access')}</span>
                    </div>
                  )}
                </div>
                ...
                >
                  {isCurrentPlan ? (
                    t('upgradePage.currentPlanLabel', 'Current Plan')
                  ) : isFree ? (
                    t('upgradePage.freeTierLabel', 'Free Tier')
                  ) : planChangeType === 'downgrade' ? (
                    <>
                      {t('upgradePage.downgrade', 'Downgrade')}
                      <ArrowRight className="h-4 w-4 ml-2" />
                    </>
                  ) : (
                    <>
                      {t('upgradePage.upgrade', 'Upgrade')}
                      <ArrowRight className="h-4 w-4 ml-2" />
                    </>
                  )}
                </Button>
      ...
      <Card className="p-6 bg-slate-50 dark:bg-slate-900/50 border-slate-200 dark:border-slate-800">
        <div className="flex items-start gap-4">
          ...
          <div className="space-y-2">
            <h3 className="text-lg font-semibold text-foreground">{t('upgradePage.creditBasedCalling', 'Credit-Based Calling')}</h3>
            <p className="text-sm text-muted-foreground">
              {t('upgradePage.creditBasedCallingDesc', 'Premium plan users can purchase credits for making calls. 1 credit = 60 seconds of call time (rounded up). For example, a 62-second call uses 2 credits.')}
            </p>
          </div>
        </div>
      </Card>
            ...
            <>
              <DialogHeader>
                <DialogTitle className="flex items-center gap-2">
                  ...
                  {t('upgradePage.subscribeTo', { name: selectedPlan.displayName, defaultValue: `Subscribe to ${selectedPlan.displayName}` })}
                </DialogTitle>
                <DialogDescription>
                  {t('upgradePage.subscribeDesc', 'Choose your currency, payment method, and billing period')}
                </DialogDescription>
              </DialogHeader>
              ...
              <div className="space-y-6 py-4">
                {availableCurrencies.length > 1 && (
                  <div className="space-y-2">
                    <label className="text-sm font-medium flex items-center gap-2">
                      ...
                      {t('upgradePage.currency', 'Currency')}
                    </label>
                    <Select
                ...
                {selectedCurrency && selectedPlan && getPlanSyncedGatewaysForCurrency(selectedPlan, selectedCurrency, billingPeriod).length > 0 && (
                  <div className="space-y-2">
                    <label className="text-sm font-medium">{t('upgradePage.paymentMethod', 'Payment Method')}</label>
                ...
                <div className="space-y-2">
                  <label className="text-sm font-medium">{t('upgradePage.billingPeriod', 'Billing Period')}</label>
                    ...
                    <SelectContent>
                      <SelectItem value="monthly">
                        {(() => {
                          ...
                          return t('upgradePage.monthlyLabel', { price: `${symbol}${parseFloat(price).toFixed(2)}`, defaultValue: `Monthly - ${symbol}${parseFloat(price).toFixed(2)}/month` });
                        })()}
                      </SelectItem>
                      {selectedPlan.yearlyPrice && (
                        <SelectItem value="yearly">
                          {(() => {
                            ...
                            return t('upgradePage.yearlyLabel', { price: `${symbol}${parseFloat(price).toFixed(2)}`, defaultValue: `Yearly - ${symbol}${parseFloat(price).toFixed(2)}/year` });
                          })()}
                        </SelectItem>
                      )}
                    </SelectContent>
                ...
                <div className="bg-slate-50 dark:bg-slate-800/50 rounded-lg p-4 border border-slate-200 dark:border-slate-700">
                  <div className="flex justify-between items-center">
                    <span className="text-sm text-muted-foreground">{t('upgradePage.total', 'Total')}</span>
                    <span className="text-2xl font-bold">
                      ...
                      <span className="text-sm font-normal text-muted-foreground">
                        /{billingPeriod === "yearly" ? t('upgradePage.year', 'year') : t('upgradePage.month', 'month')}
                      </span>
                    </span>
                  </div>
                  {selectedGateway && (
                      ...
                      {(() => {
                        ...
                        return (
                          <>
                            ...
                            <span>{t('upgradePage.securePayment', { name: info.name, defaultValue: `Secure payment via ${info.name}` })}</span>
                          </>
                        );
                      })()}
                    </div>
                ...
                >
                  {processingGateway ? (
                    <>
                      ...
                      {t('upgradePage.processing', 'Processing...')}
                    </>
                  ) : (
                    <>
                      {selectedGateway && (() => {
                      ...
                      {t('upgradePage.proceedToPayment', 'Proceed to Payment')}
                    </>
                  )}
                </Button>
                ...
                {!selectedGateway && (
                  <p className="text-xs text-muted-foreground text-center">
                    {t('upgradePage.selectPaymentMethod', 'Please select a payment method to continue.')}
                  </p>
                )}
              </div>
            </>
