...
import { MasterAIService } from '../master-ai/master-ai.service';
...
export class AudioSession extends EventEmitter {
  ...
  private masterAiService = new MasterAIService();
  ...
  private fillerAudioCache: Buffer | null = null;
  private fillerCacheText: string | null = null;
  ...
  async initialize(): Promise<void> {
    ...
    try {
      ...
      // Pre-synthesize the static greeting and filler phrases in the background.
  ...
  private async preSynthesizeFiller(): Promise<void> {
  ...
  private async *getTtsStream(text: string): AsyncIterable<Buffer> {
    ...
    if (this.fillerAudioCache && this.fillerCacheText === text) {
  ...
  processAudio(chunk: Buffer): void {
    ...
    const isWithinEchoGuardWindow = timeSinceLastTts < 100; // 100ms guard window
    ...
    if (!isWithinEchoGuardWindow) {
      ...
      // For REST-based STT providers (Sarvam) or streaming ones (Deepgram), flush on VAD speech-end
      if (vadResult.isSpeechEnd && this.sttProvider) {
        if ((this.sttProvider as any).flush) {
          ...
          // If Deepgram streaming is active, send a payload to force immediate finalize
          // Deepgram supports sending an empty json payload with CloseStream or Finalize
          console.log(`[AudioSession:${this.id}] Forcing Deepgram stream finalize on VAD silence`);
          (this.sttProvider as any).sendAudio(JSON.stringify({ type: 'Finalize' }));
        }
      }
    }
  ...
  private handleSttTranscript(transcript: SttTranscript): void {
    ...
    if (this.sttProvider?.name === 'deepgram') {
      if (!transcript.speechFinal && transcript.text.trim()) {
        // Accumulate chunk final into pendingTranscript while speaker is mid-utterance
        const chunkText = transcript.text.trim();
        if (!this.pendingTranscript.includes(chunkText)) {
          this.pendingTranscript = (this.pendingTranscript ? `${this.pendingTranscript} ${chunkText}` : chunkText).trim();
    ...
    // Speech complete (or Sarvam final / Deepgram UtteranceEnd)
    const rawText = (this.pendingTranscript ? `${this.pendingTranscript} ${transcript.text || ''}` : (transcript.text || '')).trim();
    this.pendingTranscript = '';
    const text = rawText;
    console.log(`[AudioSession:${this.id}] handleSttTranscript - FINAL UTTERANCE: "${text}"`);

    // Master AI: Smart Backchanneling & Hard Interruption filter while agent is speaking
    if (this.isPlayingTts && text) {
      if (this.masterAiService.isBackchannel(text, this.agentConfig.language)) {
        console.log(`[AudioSession:${this.id}] [MasterAI] User affirmation backchannel "${text}" during agent playback. Ignoring barge-in.`);
        return;
      }
      if (this.masterAiService.isHardInterruption(text)) {
        console.log(`[AudioSession:${this.id}] [MasterAI] Hard interruption keyword detected in "${text}". Cutting off agent playback immediately.`);
        this.handleInterruption();
      }
    }
  ...
  private async processUserUtterance(text: string): Promise<void> {
    ...
    // ─── Master AI: Zero-Latency Deterministic Action Gate ─────────────
    const masterAiConfig = this.agentConfig.masterAiConfig || {};
    const actionResult = this.masterAiService.evaluateAction(text, masterAiConfig);
    if (actionResult.action === 'hangup') {
      console.log(`[AudioSession:${this.id}] [MasterAI] Action Gate -> HANGUP triggered for "${text}"`);
    ...
    if (actionResult.action === 'transfer' && (actionResult.transferTarget || this.agentConfig.transferPhoneNumber)) {
      ...
      console.log(`[AudioSession:${this.id}] [MasterAI] Action Gate -> TRANSFER to ${targetNumber} for "${text}"`);
      this.isProcessingLlm = false;
      this.conversationMessages.push({ role: 'user', content: text });
      this.addTranscriptEntry('user', text);
      if (actionResult.speechResponse) {
    ...
    // ─── Master AI: Fast Slot Pre-Extractor ──────────────────────────
    const extractedSlots = this.masterAiService.extractSlots(text);
    ...
    // Add user message with slot context for tool parameter grounding
    ...
    try {
      ...
      if (toolCallEntries.length > 0) {
        ...
        // ─── Latency Guard: Speak a quick filler phrase to prevent dead silence during tool execution ───
  ...
  private async speakTextStreaming(text: string, preSynth?: Buffer | null): Promise<void> {
    ...
    const flush = async (block: Buffer): Promise<void> => {
      ...
      // Pace slightly faster than real time (85%) so FreeSWITCH's playback queue stays
      // filled and is not starved by Node event loop latency or network jitters.
  ...
  private findSentenceEnd(text: string): number {
    // Find sentence-ending punctuation (high priority)
    ...
    // If no full sentence end found and text is sufficiently long (>35 chars),
    // split on clause boundaries to start TTS streaming earlier
    if (lastEnd === -1 && text.length > 35) {
      const clauseEndings = [', ', '; ', ': ', ' — '];
      for (const clause of clauseEndings) {
        const idx = text.lastIndexOf(clause);
