...
export class VadDetector {
  ...
  private readonly energyWindowSize = 8;
  private noiseFloor = 0.008; // Adaptive ambient noise floor estimate
  ...
  processChunk(chunk: Buffer, isPlayingTts = false): VadResult {
    ...
    // Calculate dynamic threshold based on background noise floor
    const dynamicThreshold = Math.max(
      this.config.energyThreshold,
      this.noiseFloor * 2.0 + 0.008
    );

    // During TTS playback, require higher energy to prevent echo/leakage from triggering barge-in
