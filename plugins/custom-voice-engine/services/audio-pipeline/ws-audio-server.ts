...
export class AudioWebSocketServer {
  ...
  private async handleConnection(ws: WebSocket, req: IncomingMessage): Promise<void> {
    try {
      ...
      // Set up audio output: send directly via WS for widgets, save file to disk and play via ESL for FreeSWITCH!
      ...
      if (!session.isInitialized()) {
        ...
        if (session.direction === 'inbound') {
          ...
          // Check if call was already answered before WebSocket connected
  ...
  private async resolveSessionDynamically(sessionId: string): Promise<AudioSession | null> {
    try {
      ...
      if (!targetAgentId && toNumber) {
        ...
        if (!targetAgentId) {
          // Query sip_phone_numbers mapping directly (e.g. for Twilio SIP, ElevenLabs SIP, OpenAI SIP)
          connResult = await db.execute(sql`
      ...
      // Merge overrides from session metadata
...
${agent.systemPrompt || ''}`;
      ...
      // Add System Tools (Transfer Call)
      ...
      // Add System Tools (Appointment Booking)
      if (agent.appointmentBookingEnabled) {
        const callerPhone = (sessionData?.direction === 'outbound')
          ? (toNumber || sessionData?.toNumber || '')
          : (fromNumber || sessionData?.fromNumber || '');
        tools.push({
          type: 'function',
          function: {
      ...
      let llmApiKey = llmProvider === 'gemini'
        ? (globalSettingsMap['ve_gemini_api_key'] || process.env.GEMINI_API_KEY || '')
        : (globalSettingsMap['ve_openrouter_api_key'] || process.env.OPENROUTER_API_KEY || '');
      ...
      // If tenant has an explicit api key override for the effective provider, prefer that (if BYOK is allowed).
...
You MUST speak ONLY in ${languageName}. From the very first word you say, speak in ${languageName}. Do NOT speak English or any other language. This overrides any previous language instructions in this prompt. This is mandatory.`;
      ...
      const finalLlmApiKey = (providerConfig && (providerConfig.llmProvider === effectiveLlmProvider || (effectiveLlmProvider === 'gemini' && providerConfig.llmProvider === 'gemini')) && providerConfig.llmApiKey)
        ? providerConfig.llmApiKey
      ...
      // Deduplicate tools to prevent duplicate function declarations (e.g., from flow compiled tools vs system tools)
      const uniqueTools: LlmToolDefinition[] = [];
      ...
      const isByok = allowUserByok && Boolean(
        (providerConfig?.sttApiKey && providerConfig?.sttProvider === effectiveSttProvider) ||
        (providerConfig?.llmApiKey && (providerConfig?.llmProvider === effectiveLlmProvider || (effectiveLlmProvider === 'gemini' && providerConfig?.llmProvider === 'gemini'))) ||
        (providerConfig?.ttsApiKey && providerConfig?.ttsProvider === effectiveTtsProvider)
      );
