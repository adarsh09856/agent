...
import { appointments, agents } from '../../../../shared/schema';
...
export class ToolExecutor {
  ...
  static async executeSendWhatsApp(
    ...
    try {
      let phoneNumber = (params.phone_number as string) || (params.phoneNumber as string) || '';
      const template = (params.template_name as string) || (params.templateName as string) || metadata?.templateName || 'hello_world';
      ...
      // Sanitize "plus" or spaces if spoken like "plus 9 1 ..."
      if (phoneNumber) {
      ...
      let templateVariables = (params.template_variables as any[]) || (params.templateVariables as any[]) || [];
  ...
  static async executeSendEmail(
    ...
    try {
      const recipientEmail = (params.recipient_email as string) || (params.recipientEmail as string) || metadata?.recipientEmail;
      const template = (params.template_name as string) || (params.templateName as string) || metadata?.templateName || 'default';
      ...
      const dynamicVars = (params.dynamic_variables as Record<string, string>) || (params.dynamicVariables as Record<string, string>) || {};
  ...
  static getAppointmentWebhookSecret(): string {
