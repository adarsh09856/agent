(function() {
  ...
  async function startCall() {
    ...
    try {
      ...
      } else if (currentEngine === 'custom-voice-engine' && tokenData.ws_url) {
        console.log('VoiceWidget: Using Custom Voice Engine with WS URL:', tokenData.ws_url);
        await initCustomVoiceEngine(tokenData.ws_url);
      } else {
  ...
  async function initCustomVoiceEngine(wsUrl) {
    return new Promise(function(resolve, reject) {
      elevenLabsWs = new WebSocket(wsUrl);
      elevenLabsWs.binaryType = 'arraybuffer';
      
      audioContext = new (window.AudioContext || window.webkitAudioContext)();
      analyser = audioContext.createAnalyser();
      analyser.fftSize = 256;
      
      var SOURCE_SAMPLE_RATE = 8000;
      var targetSampleRate = audioContext.sampleRate;
      console.log('VoiceWidget: AudioContext sample rate for CVE:', targetSampleRate);
      ...
      function sendAudioToCustomVoiceEngine(float32Data) {
        ...
        elevenLabsWs.send(pcm16.buffer);
        
        audioSendCount++;
        if (audioSendCount === 1 || audioSendCount % 100 === 0) {
          console.log('VoiceWidget CVE: Audio #' + audioSendCount + ' | RMS: ' + rms.toFixed(4) + ' | Method: ' + audioCaptureMethod + ' | SR: ' + inputRate + ' | Samples: ' + downsampled.length);
        }
      }
      
      function setupScriptProcessor() {
        console.log('VoiceWidget CVE: Setting up ScriptProcessor for audio capture');
        ...
        inputProcessor.onaudioprocess = function(e) {
          ...
          sendAudioToCustomVoiceEngine(new Float32Array(inputData));
        };
        console.log('VoiceWidget CVE: ScriptProcessor audio capture active, bufferSize:', bufferSize);
      }
      
      if (mediaStream) {
        inputSource = audioContext.createMediaStreamSource(mediaStream);
        console.log('VoiceWidget CVE: MediaStreamSource created');
        console.log('VoiceWidget CVE: AudioContext state:', audioContext.state, '| sampleRate:', audioContext.sampleRate);
        
        if (audioContext.state === 'suspended') {
          console.log('VoiceWidget CVE: AudioContext suspended, resuming...');
          audioContext.resume().then(function() {
            console.log('VoiceWidget CVE: AudioContext resumed, state:', audioContext.state);
          });
        }
        
        if (audioContext.audioWorklet) {
          console.log('VoiceWidget CVE: AudioWorklet supported, attempting to load module...');
          audioContext.audioWorklet.addModule(
            ...
            inputSource.connect(inputProcessor);
            console.log('VoiceWidget CVE: AudioWorklet audio capture active, bufferSize: 4096');
            
            inputProcessor.port.onmessage = function(event) {
              sendAudioToCustomVoiceEngine(event.data.audioData);
            };
          }).catch(function(e) {
            console.log('VoiceWidget CVE: AudioWorklet failed (' + e.message + '), using ScriptProcessor fallback');
            setupScriptProcessor();
          });
        } else {
          console.log('VoiceWidget CVE: AudioWorklet not supported, using ScriptProcessor');
          setupScriptProcessor();
        }
      } else {
        console.log('VoiceWidget CVE: No mediaStream available for audio capture');
      }
      
      elevenLabsWs.onopen = function() {
        console.log('VoiceWidget CVE: Custom Voice Engine WebSocket connected');
        resolve();
      };
        ...
        if (event.data instanceof ArrayBuffer) {
          ...
          try {
            ...
            if (msg.type === 'interruption') {
              console.log('VoiceWidget CVE: Interruption received, clearing audio queue');
              audioQueue.length = 0;
              isPlaying = false;
            } else if (msg.type === 'hangup') {
              console.log('VoiceWidget CVE: Session ended by agent');
              endCall();
            }
          } catch (e) {
            console.log('VoiceWidget CVE: Could not parse WS message', e);
          }
        }
      };
      
      elevenLabsWs.onerror = function(error) {
        console.error('VoiceWidget CVE: WebSocket error', error);
        reject(new Error('Custom Voice Engine connection failed'));
      };
      
      elevenLabsWs.onclose = function(event) {
        console.log('VoiceWidget CVE: WebSocket closed. Code:', event.code, 'Reason:', event.reason || 'No reason specified');
        if (state === 'active') {
          endCall();
        }
      };
  ...
  async function initElevenLabsWebSocket(signedUrl, languageToUse) {
    return new Promise(function(resolve, reject) {
      ...
      elevenLabsWs.onopen = function() {
        ...
        // Only include conversation_config_override if language is specified and not 'en'
        // to prevent immediate WebSocket termination due to ElevenLabs policy restriction
        // (by default, ElevenLabs restricts overrides unless explicitly enabled in dashboard security settings)
        if (languageToUse && languageToUse !== 'en') {
          ...
          console.log('VoiceWidget: Sent conversation_initiation_client_data with language override:', languageToUse);
        } else {
          console.log('VoiceWidget: Sent conversation_initiation_client_data (no language override)');
        }
