...
const targetPlugins = process.argv.slice(2);
let plugins = fs.readdirSync(pluginsDir, { withFileTypes: true })
...
if (targetPlugins.length > 0) {
  plugins = plugins.filter(p => targetPlugins.includes(p));
  if (plugins.length === 0) {
    console.log(`No matching plugins found for: ${targetPlugins.join(', ')}`);
    process.exit(0);
  }
  console.log(`Building only specified plugin(s): ${plugins.join(', ')}`);
} else {
