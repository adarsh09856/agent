...
import { hasActiveMembership, syncUserWithSubscription, applyPlanCredits } from '../../../../services/membership-service';
...
router.post('/confirm-subscription', authenticateToken, async (req: AuthRequest, res: Response) => {
  try {
    ...
    // Apply plan credits (included credits for the plan)
    await applyPlanCredits(userId, plan.id, 'mercadopago', subscriptionId);
