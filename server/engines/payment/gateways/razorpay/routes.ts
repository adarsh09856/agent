...
import { hasActiveMembership, applyPlanCredits } from '../../../../services/membership-service';
...
router.post('/verify-subscription', authenticateToken, async (req: AuthRequest, res: Response) => {
  try {
    ...
    // Apply plan credits (e.g. 700 credits for Scale plan)
    await applyPlanCredits(userId, plan.id, 'razorpay', razorpay_payment_id);
