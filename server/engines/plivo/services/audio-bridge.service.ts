...
export interface AudioBridgeSession {
  ...
  silenceWarningSent?: boolean;
  silenceTimer?: NodeJS.Timeout | null;
  autoEndCallPending?: boolean;
}
...
5. These function calls are MANDATORY. Data will NOT be saved unless you call the functions.`;
    ...
    try {
      ...
      switch (message.type) {
        ...
        case 'response.output_audio.done':
          ...
          if (session.autoEndCallPending) {
            logger.info(`Auto-ending call post-task completion for ${callUuid}...`, undefined, 'AudioBridge');
            setTimeout(() => {
              this.disconnectSession(callUuid);
            }, 1500);
          }
          break;
        ...
        case 'input_audio_buffer.speech_started':
          ...
          session.lastActivityTimestamp = Date.now();
          session.silenceWarningSent = false;
    ...
    try {
      ...
      if (typeof result === 'object' && result !== null) {
        ...
        if (toolName === 'book_appointment' && toolResult.success === true) {
          session.autoEndCallPending = true;
  ...
  private static handleBargeIn(session: AudioBridgeSession): void {
    ...
    if (plivoWs && plivoWs.readyState === WebSocket.OPEN) {
  ...
  static async endSession(callUuid: string): Promise<{
    ...
    logger.info(`End session called for ${callUuid}`, undefined, 'AudioBridge');
  ...
  private static startSilenceMonitor(session: AudioBridgeSession) {
    session.lastActivityTimestamp = Date.now();
    session.silenceWarningSent = false;
    ...
    session.silenceTimer = setInterval(() => {
      ...
      const now = Date.now();
      const silenceDuration = now - (session.lastActivityTimestamp || now);

      if (silenceDuration > 22000 && session.silenceWarningSent) {
        logger.info(`Prolonged silence timeout (22s) for ${session.callUuid}. Disconnecting.`, undefined, 'AudioBridge');
        this.injectMessageToOpenAI(session, "No response detected, disconnecting call. Goodbye.");
        setTimeout(() => this.disconnectSession(session.callUuid), 2000);
        if (session.silenceTimer) clearInterval(session.silenceTimer);
      } else if (silenceDuration > 12000 && !session.silenceWarningSent) {
        logger.info(`Silence warning (12s) for ${session.callUuid}.`, undefined, 'AudioBridge');
        session.silenceWarningSent = true;
        this.injectMessageToOpenAI(session, "Are you there? If you are unable to respond, I will disconnect the call shortly.");
      }
    }, 2000);
  }

  private static injectMessageToOpenAI(session: AudioBridgeSession, text: string) {
  ...
  private static disconnectSession(callUuid: string) {
    this.endSession(callUuid);
    const session = this.activeSessions.get(callUuid) || { callUuid } as any as AudioBridgeSession;
    this.executeHangup(session).catch(err => {
      logger.error(`Error executing hangup during disconnectSession for ${callUuid}`, err, 'AudioBridge');
    });
  }
}
