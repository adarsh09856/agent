...
async function initializeSession(
  ...
  try {
    ...
    if (agentConfig.model !== callRecord.openaiModel) {
      await db
        .update(twilioOpenaiCalls)
        .set({
          ...
          metadata: {
            ...(metadata || {}),
            originalModelRequested: callRecord.openaiModel,
            modelFallbackApplied: true
          }
        })
        .where(eq(twilioOpenaiCalls.id, callRecord.id));
      logger.warn(`[TwilioOpenAI Stream] Fallback model applied for call ${callRecord.id}: requested "${callRecord.openaiModel}" → using "${agentConfig.model}"`, undefined, 'TwilioOpenAI Stream');
    }
