...
function getEngineLanguages(engine: 'elevenlabs' | 'openai' | 'custom-voice-engine' | 'none'): typeof ELEVENLABS_LANGUAGES {
  return engine === 'elevenlabs' ? ELEVENLABS_LANGUAGES : OPENAI_LANGUAGES;
}

// Get languages supported by the specific agent
function getAgentSupportedLanguages(agent: any, engine: 'elevenlabs' | 'openai' | 'custom-voice-engine' | 'none'): typeof ELEVENLABS_LANGUAGES {
...
async function getAgentEngineType(agentId: string | null): Promise<{ engine: 'elevenlabs' | 'openai' | 'custom-voice-engine' | 'none', agent: any }> {
  if (!agentId) {
    ...
    try {
      const veResult = await db.execute(sql`
        SELECT 
          ...
          user_id as "userId"
        FROM ve_voice_agents WHERE id = ${agentId} LIMIT 1
      `);
      if (veResult.rows.length > 0) {
        agent = veResult.rows[0];
        agent.telephonyProvider = 'custom-voice-engine';
      }
    } catch (err: any) {
      console.error(`[Widget] Failed to query ve_voice_agents fallback:`, err);
    }
  }
  ...
  if (agent.telephonyProvider === 'custom-voice-engine') {
    return { engine: 'custom-voice-engine', agent };
  }
...
router.post('/widget/session/:sessionId/ephemeral-token', async (req: Request, res: Response) => {
  try {
    ...
    if (engine === 'custom-voice-engine' && agent) {
      console.log(`[Widget] Using Custom Voice Engine for agent ${agent.name}`);
      
      try {
        // Pre-create the session in ve_sessions table so that the websocket handler finds it
        await db.execute(sql`
          INSERT INTO ve_sessions (
            id, user_id, agent_id, from_number, to_number, direction, status, channel_uuid
          ) VALUES (
            ${session.id}, ${widget.userId}, ${agent.id}, 'WebWidget', 'Front Desk', 'inbound', 'initializing', ${session.id}
          )
        ...
        const wsUrl = `${protocol}://${host}/voice-engine/audio/${session.id}?client=widget`;
        ...
        console.error('[Widget] Custom Voice Engine session setup error:', cveError);
        return res.status(500).json({ error: 'Failed to connect to Custom Voice Engine. Please try again.' });
      }
    ...
    const rawModel = agent?.openaiModel || 'gpt-realtime-1.5';
...
router.post('/widget/session/:sessionId/book-appointment', async (req: Request, res: Response) => {
  try {
    ...
    // Normalize appointmentDate to YYYY-MM-DD format
    ...
    if (isoDateMatch) {
      ...
      console.log(`[Widget] Date already in ISO format: ${appointmentDate}`);
    } else {
      // Parse natural language date using chrono-node
      const chronoResult = chrono.parseDate(appointmentDate, new Date(), { forwardDate: true });
      if (chronoResult) {
        parsedDate = chronoResult;
        normalizedDate = formatDate(chronoResult, 'yyyy-MM-dd');
        console.log(`[Widget] Parsed natural language date: "${appointmentDate}" -> ${normalizedDate}`);
      }
    }
    ...
    if (!normalizedDate || !parsedDate) {
      ...
      if (lowerDate === 'tomorrow') {
        ...
        normalizedDate = formatDate(parsedDate, 'yyyy-MM-dd');
      ...
      if (normalizedDate) {
        console.log(`[Widget] Parsed relative date: "${appointmentDate}" -> ${normalizedDate}`);
      }
    }
    ...
    if (!normalizedDate || !parsedDate) {
      return res.status(400).json({ 
        success: false, 
        message: `Invalid date format: "${appointmentDate}". Please provide a date like "tomorrow", "December 5th", or "2024-01-15".` 
      });
    }
    
    // Normalize appointmentTime to HH:MM format (24-hour)
    let normalizedTime = appointmentTime.trim();
    // Remove any seconds or AM/PM variations and ensure 24-hour format
    ...
    // Create scheduledAt from normalized date and time
    ...
    if (settings?.workingHours) {
      ...
      if (daySettings && daySettings.enabled) {
        if (normalizedTime < daySettings.start || normalizedTime >= daySettings.end) {
          return res.status(400).json({ 
            success: false, 
