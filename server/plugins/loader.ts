...
export interface PluginLoaderOptions {
  ...
  callServices?: any; // Passed dynamically
}
        ...
        try {
          ...
          await db.execute(sql.raw(`
            CREATE TABLE IF NOT EXISTS plugin_migrations (
              ...
              PRIMARY KEY (plugin_name, migration_file)
            )
          `));

          // Fetch already applied migrations for this plugin
          const appliedResult = await db.execute(sql`
            SELECT migration_file FROM plugin_migrations WHERE plugin_name = ${manifest.name}
          `);
          const appliedMigrations = new Set(
            (appliedResult.rows as Array<{ migration_file: string }>).map(r => r.migration_file)
          );

          const migrations = manifest.database?.migrations || [];
          const unappliedMigrations = migrations.filter((m: string) => !appliedMigrations.has(m));

          if (missingTables.length > 0 || unappliedMigrations.length > 0) {
            if (missingTables.length > 0) {
              console.log(`[Plugin Loader] Plugin '${manifest.name}' has missing tables: ${missingTables.join(', ')}`);
            }
            if (unappliedMigrations.length > 0) {
              console.log(`[Plugin Loader] Plugin '${manifest.name}' has unapplied migrations: ${unappliedMigrations.join(', ')}`);
            }
            ...
            if (migrations.length > 0) {
              ...
              for (const migrationFile of migrations) {
                // If the migration has already been applied and we don't have missing tables, skip it
                if (missingTables.length === 0 && appliedMigrations.has(migrationFile)) {
                  continue;
                }
                ...
                if (fs.existsSync(migrationPath)) {
                  ...
                  try {
                    ...
                    await db.execute(sql`
                      ...
                      VALUES (${manifest.name}, ${migrationFile})
                      ON CONFLICT (plugin_name, migration_file) DO NOTHING
                    `);
