...
export function registerConnectionsRoutes(router: Router) {
  ...
  router.post('/test-connection/deepgram', requireAdminPermission('settings', 'system_settings', 'update'), async (req: AdminRequest, res: Response) => {
    try {
      const dbKey = await storage.getGlobalSetting('ve_deepgram_api_key');
      const apiKey = (dbKey?.value as string) || process.env.DEEPGRAM_API_KEY;

      if (!apiKey) {
        return res.json({ connected: false, error: 'Deepgram API key not configured' });
      }
      ...
      if (response.ok) {
        const projectCount = isJson && data?.projects ? data.projects.length : 1;
        res.json({
          connected: true,
          details: 'Connected to Deepgram (Nova-2 STT & Aura TTS)',
          projectCount,
          source: dbKey?.value ? 'Voice Engine Settings' : 'Environment Variable'
        });
      } else {
        res.json({ connected: false, error: `Deepgram API error: HTTP ${response.status}` });
      }
    } catch (error: any) {
      res.json({ connected: false, error: error.message || 'Failed to test Deepgram connection' });
    }
  });

  router.post('/test-connection/gemini', requireAdminPermission('settings', 'system_settings', 'update'), async (req: AdminRequest, res: Response) => {
    try {
      const dbKey = await storage.getGlobalSetting('ve_gemini_api_key');
      const apiKey = (dbKey?.value as string) || process.env.GEMINI_API_KEY;

      if (!apiKey) {
        return res.json({ connected: false, error: 'Google Gemini API key not configured' });
      }

      const response = await fetchWithTimeout(`https://generativelanguage.googleapis.com/v1beta/models/gemini-1.5-flash?key=${apiKey}`);
      const { data, isJson } = await safeParseResponse(response);

      if (response.ok) {
        res.json({
          connected: true,
          details: 'Connected to Google Gemini (Flash 2.0 / 1.5)',
          model: data?.name || 'gemini-1.5-flash',
          source: dbKey?.value ? 'Voice Engine Settings' : 'Environment Variable'
        });
      } else {
        res.json({ connected: false, error: `Gemini API error: HTTP ${response.status}` });
      }
    } catch (error: any) {
      res.json({ connected: false, error: error.message || 'Failed to test Gemini connection' });
    }
  });
  ...
  router.post('/test-connection/yookassa', requireAdminPermission('settings', 'system_settings', 'update'), async (req: AdminRequest, res: Response) => {
    try {
      const dbShopId = await storage.getGlobalSetting('yookassa_shop_id');
      const dbSecretKey = await storage.getGlobalSetting('yookassa_secret_key');
      
      const shopId = dbShopId?.value as string;
      const secretKey = dbSecretKey?.value as string;
      
      if (!shopId || !secretKey) {
        return res.json({ connected: false, error: 'YooKassa credentials not configured' });
      }
      
      const auth = Buffer.from(`${shopId}:${secretKey}`).toString('base64');
      const response = await fetchWithTimeout('https://api.yookassa.ru/v3/payments?limit=1', { headers: { 'Authorization': `Basic ${auth}` } });
      const { data, isJson, rawText } = await safeParseResponse(response);
      
      if (response.ok) {
        res.json({ connected: true, source: 'Database' });
      } else {
        res.json({ connected: false, error: `YooKassa API error: ${isJson && data?.description ? data.description : response.statusText}` });
      }
    } catch (error: any) {
      const isTimeout = error.name === 'AbortError';
      res.json({ connected: false, error: isTimeout ? 'Connection timed out.' : (error.message || 'Failed to test YooKassa connection') });
    }
  });
