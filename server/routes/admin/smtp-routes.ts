...
export function registerSmtpRoutes(router: Router) {
  ...
  router.post('/smtp/test', requireAdminPermission('communications', 'email_settings', 'update'), async (req: AdminRequest, res: Response) => {
    try {
      ...
      } else if (errorCode === 'EAI_AGAIN' || errorCode === 'ENOTFOUND' || errorMsg.includes('ENOTFOUND') || errorMsg.includes('EAI_AGAIN')) {
        friendlyMessage = `SMTP server hostname could not be resolved. Please check that the hostname is spelled correctly (ensure there are no spaces or typos like "SMTP-server relay-brevo.com" instead of "smtp-relay.brevo.com").`;
