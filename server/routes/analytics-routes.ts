...
export function createAnalyticsRoutes(ctx: RouteContext): Router {
  ...
  router.get("/api/calls", authenticateHybrid, async (req: AuthRequest, res: Response) => {
    try {
      ...
      if (requestsPagination) {
        ...
        const paginatedResponse = await storage.getUserCallsPaginated(req.userId!, {
          page,
          pageSize,
          search,
          status,
  ...
  router.get("/api/calls/:id/recording", authenticateHybrid, async (req: AuthRequest, res: Response) => {
    try {
      ...
      // Handle Custom Voice Engine calls (local recording file)
      if (callWithDetails.engine === 'custom-voice-engine') {
        console.log(`🎙️ [Recording] Fetching Custom Voice Engine recording for session ${callWithDetails.id}`);
        
        // Find the recording details in ve_call_recordings table
        const result = await db.execute(sql`
          SELECT storage_path, status, storage_url FROM ve_call_recordings WHERE session_id = ${callWithDetails.id} LIMIT 1
        `);
        
        console.log(`🎙️ [Recording] Database query completed. Found ${result.rows.length} recording records in ve_call_recordings.`);
        
        if (result.rows.length > 0) {
          const row = result.rows[0] as any;
          console.log(`🎙️ [Recording] Found record. status: "${row.status}", storage_path: "${row.storage_path}", storage_url: "${row.storage_url}"`);
          
          if (row.status !== 'available') {
            console.warn(`🎙️ [Recording] Recording for session ${callWithDetails.id} is not marked as 'available' (current status: ${row.status})`);
          }
          
          const storagePath = row.storage_path;
          if (fs.existsSync(storagePath)) {
            console.log(`🎙️ [Recording] File exists on disk. Reading and sending: ${storagePath}`);
            const audioBuffer = fs.readFileSync(storagePath);
            res.setHeader('Content-Type', 'audio/wav');
            res.setHeader('Content-Disposition', `inline; filename="call-recording-${callWithDetails.id}.wav"`);
            res.setHeader('Cache-Control', 'no-cache');
            return res.send(audioBuffer);
          } else {
            console.error(`🎙️ [Recording] ERROR: File path registered in database does not exist on disk: "${storagePath}"`);
          }
        } else {
          console.warn(`🎙️ [Recording] No database entry found in ve_call_recordings for session ${callWithDetails.id}`);
        }
        
        return res.status(404).json({
          error: "Recording not available",
          details: "No recording found for this Custom Voice Engine session. The file might have been deleted or the call was not recorded."
        });
      }
  ...
  router.post("/api/analytics/export-pdf", authenticateHybrid, async (req: AuthRequest, res: Response) => {
    try {
      ...
      // Global Page numbering and footer
      const range = doc.bufferedPageRange();
      for (let i = 0; i < range.count; i++) {
        doc.switchToPage(i);
        
        // Temporarily clear bottom margin to prevent auto-page break during footer rendering
        const oldBottomMargin = doc.page.margins.bottom;
        doc.page.margins.bottom = 0;
        
        const footerY = 805;
        doc.save()
          .rect(0, footerY, 595, 37)
          .fill(colors.background);
        doc.restore();
        ...
        doc.fillColor(colors.textMuted)
          .fontSize(8)
          .font('Helvetica')
          .text(`${appName} Analytics Report`, leftMargin, footerY + 12, { width: pageWidth / 2 });
        
        doc.text(`Page ${i + 1} of ${range.count}`, leftMargin + pageWidth - 80, footerY + 12, { width: 80, align: 'right' });
