...
export function createCampaignRoutes(ctx: RouteContext): Router {
  ...
  function canAccessCampaign(campaign: any, req: AuthRequest): boolean {
    if (!campaign) return false;
    if (!(req as any).userId && !(req as any).user) return true;
    if (campaign.userId === (req as any).userId || campaign.userId === (req as any).user?.id) return true;
    if (['admin', 'manager', 'superadmin'].includes((req as any).user?.role || '')) return true;
    if ((req as any).user?.isTeamMember || (req as any).isTeamMember) return true;
    return false;
  }
    ...
    try {
      ...
      if (sipPhoneNumberId) {
        if (agent.telephonyProvider === 'custom-voice-engine') {
          const [userSipPhoneNumber] = (await db.execute(sql`
            SELECT * FROM user_sip_phone_numbers WHERE id = ${sipPhoneNumberId} LIMIT 1
          `)).rows as any[];

          if (!userSipPhoneNumber) {
            // Check standard sip_phone_numbers
            ...
            if (!sipPhoneNumber) {
              return res.status(404).json({ error: "Custom/standard SIP phone number not found" });
            }
            ...
            // Validate ownership
            if (userSipPhoneNumber.user_id !== req.userId) {
              return res.status(403).json({ error: "You don't have access to this custom SIP phone number" });
            }
          }
        } else {
          ...
          // Validate engine compatibility with agent (Custom Voice Engine supports all)
      ...
      const campaign = await storage.createCampaign({
        ...
        scheduleTimeStart: scheduleTimeStart || null,
  ...
  router.get("/api/campaigns/:id", authenticateHybrid, async (req: AuthRequest, res: Response) => {
    try {
      ...
      if (!canAccessCampaign(campaign, req)) {
        return res.status(404).json({ error: "Campaign not found" });
      }
  ...
  router.get("/api/campaigns/:id/export", authenticateHybrid, async (req: AuthRequest, res: Response) => {
    try {
      const campaign = await storage.getCampaign(req.params.id);
      if (!campaign || !canAccessCampaign(campaign, req)) {
        return res.status(404).json({ error: "Campaign not found" });
      }
  ...
  router.patch("/api/campaigns/:id", authenticateHybrid, async (req: AuthRequest, res: Response) => {
    try {
      ...
      if (sipPhoneNumberId) {
        ...
        let isCustomVoiceEngine = false;
        if (targetAgentId) {
          const agent = await storage.getAgent(targetAgentId);
          if (agent?.telephonyProvider === 'custom-voice-engine') {
            isCustomVoiceEngine = true;
          }
        }

        let sipPhone: any = null;

        if (isCustomVoiceEngine) {
          const [userSipPhoneNumber] = (await db.execute(sql`
            SELECT * FROM user_sip_phone_numbers WHERE id = ${sipPhoneNumberId} AND user_id = ${req.userId!} LIMIT 1
          `)).rows as any[];

          if (!userSipPhoneNumber) {
            const [foundSipPhone] = await db
              .select()
              .from(sipPhoneNumbers)
              .where(and(
        ...
        if (targetAgentId) {
          const agent = await storage.getAgent(targetAgentId);
          if (agent?.telephonyProvider && agent?.telephonyProvider !== 'custom-voice-engine' && sipPhone && sipPhone.engine !== agent.telephonyProvider) {
            return res.status(400).json({ 
              error: "Engine mismatch",
      ...
      if (updateData.daysOfWeek !== undefined && updateData.scheduleDays === undefined) {
        updateData.scheduleDays = updateData.daysOfWeek;
      }
      delete updateData.daysOfWeek;

      await storage.updateCampaign(req.params.id, updateData);
  ...
  router.get("/api/campaigns/:campaignId/batch", authenticateHybrid, async (req: AuthRequest, res: Response) => {
    try {
      ...
      // Use campaignExecutor.getBatchJobStatus which handles all engines correctly and synthesizes status when needed:
      const batchJob = await campaignExecutor.getBatchJobStatus(req.params.campaignId);
      console.log(`[Campaign Batch Debug] ID: ${req.params.campaignId}, BatchJob: ${batchJob ? JSON.stringify(batchJob).substring(0, 100) : 'null'}`);
      if (!batchJob) {
      ...
      res.status(500).json({ error: "Failed to get batch job status", details: error?.message || String(error) });
    }
  });
  ...
  router.post("/api/campaigns/:campaignId/batch/retry", authenticateHybrid, async (req: AuthRequest, res: Response) => {
    try {
      ...
      const { isNonElevenLabsBatchJob } = await import('../services/campaign-executor');
      if (isNonElevenLabsBatchJob(campaign.batchJobId) || agent.telephonyProvider === 'openai-sip') {
        await storage.updateCampaign(campaign.id, {
          status: 'running',
          batchJobStatus: 'in_progress',
        });
        return res.json({ success: true, result: { status: 'in_progress' } });
      }
  ...
  router.post("/api/campaigns/:id/execute", authenticateHybrid, async (req: AuthRequest, res: Response) => {
    try {
      ...
      if ((campaign as any).sipPhoneNumberId) {
        ...
        if (sipPhone) {
          hasSipPhoneNumber = true;
        } else {
          // Fallback to user_sip_phone_numbers table
          const userSipPhoneResult = await db.execute(sql`
            SELECT id FROM user_sip_phone_numbers 
            WHERE id = ${(campaign as any).sipPhoneNumberId} AND user_id = ${req.userId!} 
            LIMIT 1
          `);
          if (userSipPhoneResult.rows.length > 0) {
            hasSipPhoneNumber = true;
          }
        }
      }
      ...
      const userSubMinutes = Number((user as any).subscriptionMinutes) || 0;
      const userWalletCredits = Number(user.credits) || 0;
      const totalAvailable = userWalletCredits + userSubMinutes;

      let isByok = false;
      try {
        const { isByokAllowed } = await import("../services/user-provider-client");
        const allowed = await isByokAllowed();
        if (allowed) {
          const credRes = await db.execute(sql`
            SELECT is_verified FROM user_provider_credentials
            WHERE user_id = ${user.id} AND provider = 'twilio' AND is_verified = true LIMIT 1
          `);
        ...
        // ignore fallback to balance
      }

      if (!isByok && totalAvailable < estimatedCredits) {
        return res.status(402).json({ 
          error: "Insufficient credits",
          message: `You need approximately ${estimatedCredits} credits/minutes to run this campaign (${totalContacts} contacts × ~2 min/call). You currently have ${totalAvailable} total (${userSubMinutes} subscription plan minutes + ${userWalletCredits} wallet credits). Please purchase more credits or upgrade your plan to continue.`,
          required: estimatedCredits,
          available: totalAvailable,
          subscriptionMinutes: userSubMinutes,
          credits: userWalletCredits
        });
      }
