        ...
        if (migrations.length > 0 && requiredTables.length > 0) {
          ...
          try {
            ...
            const appliedResult = await db.execute(sql`
              SELECT migration_file FROM plugin_migrations WHERE plugin_name = ${pluginName}
            `);
          ...
          if (needsMigration || unappliedMigrations.length > 0) {
            let migrationSuccess = true;
            for (const migFile of migrations) {
              // Skip if migration is already applied and we don't have missing tables
              if (!needsMigration && appliedMigrations.has(migFile)) {
                continue;
              }
            ...
            steps.push({ step: 'migrations', status: 'skipped', message: 'All migrations already applied and all required tables exist' });
          }
