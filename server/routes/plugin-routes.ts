  ...
  try {
    ...
    if (userId && sipPluginEnabled) {
      try {
        ...
        // Admin and Superadmin accounts get full SIP access automatically if plugin is installed
        if (userRole === 'admin' || userRole === 'super_admin') {
          userHasSipAccess = true;
          if (sipEnginesAllowed.length === 0) {
            sipEnginesAllowed = ['asterisk', 'freeswitch', 'kamailio', 'opensips', 'custom'];
          }
          if (maxConcurrentSipCalls === 0) {
            maxConcurrentSipCalls = 100;
          }
        }
      } catch (err) {
    ...
    // SIP Engine is accessible only if plugin is enabled AND user's plan allows it (or user is admin)
    ...
    res.json({
      ...
      data: {
        ...
        customVoiceEngine: capabilities['custom-voice-engine'] ?? false,
      }
    });
  } catch (error: any) {
...
publicPluginRouter.get('/:name/bundle', async (req, res) => {
  try {
    ...
    // Disable caching to ensure client always loads the latest compiled bundle
