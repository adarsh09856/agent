...
// Pending call initiation failures cache — handles race condition where
// call_initiation_failure webhook arrives before the SIP call record is created.
// Key: conversation_id, Value: { failure_reason, metadata, timestamp }
const pendingCallFailures = new Map<string, { failure_reason: string; metadata: any; timestamp: number }>();

// Auto-clean pending failures after 5 minutes
setInterval(() => {
  const cutoff = Date.now() - 5 * 60 * 1000;
  for (const [key, value] of Array.from(pendingCallFailures.entries())) {
    if (value.timestamp < cutoff) pendingCallFailures.delete(key);
  }
}, 60_000);

/**
 * Check if a conversation_id has a pending failure (race condition handler).
 * Returns the failure reason if found, null otherwise.
 */
export function consumePendingFailure(conversationId: string): { failure_reason: string; metadata: any } | null {
  const pending = pendingCallFailures.get(conversationId);
  if (pending) {
    pendingCallFailures.delete(conversationId);
    return { failure_reason: pending.failure_reason, metadata: pending.metadata };
  }
  return null;
}

async function linkPendingAppointments(params: {
  userId: string;
  callId: string;
  ...
  const { userId, callId, conversationId, phoneNumber, isSipCall } = params;
  try {
    if (conversationId) {
      const result = await db.execute(sql`
        UPDATE appointments 
        SET call_id = ${callId}, updated_at = NOW()
        WHERE user_id = ${userId}
          AND call_id IS NULL
          AND (metadata->>'elevenLabsConversationId' = ${conversationId} OR metadata->>'conversationId' = ${conversationId})
      `);
      const rowCount = result.rowCount || 0;
      if (rowCount > 0) {
        console.log(`📅 [Appointment Linker] Linked ${rowCount} appointment(s) to call ${callId} via conversationId: ${conversationId}`);
        const table = isSipCall ? 'sip_calls' : 'calls';
        await db.execute(sql`
          UPDATE ${sql.identifier(table)}
          SET metadata = COALESCE(metadata, '{}'::jsonb) || ${JSON.stringify({ appointmentBooked: true, hasAppointment: true })}::jsonb
          WHERE id = ${callId}
        `);
        return;
      }
    ...
    if (phoneNumber) {
      ...
      if (digits.length >= 6) {
        ...
        const recentAppts = await db.execute(sql`
          SELECT id, contact_phone FROM appointments
          WHERE user_id = ${userId}
            AND call_id IS NULL
            AND created_at > NOW() - INTERVAL '10 minutes'
        `);

        for (const row of recentAppts.rows as any[]) {
          const apptDigits = (row.contact_phone || '').replace(/[^0-9]/g, '');
          const apptLast10 = apptDigits.slice(-10);

          const phoneMatches = apptLast10 === last10;
          const fuzzyPhoneMatch = apptLast10.length === 10 && last10.length === 10 &&
            apptLast10.split('').filter((d: string, i: number) => d === last10[i]).length >= 9;

          if (phoneMatches || fuzzyPhoneMatch) {
            console.log(`📅 [Appointment Linker] Linked appointment ${row.id} to call ${callId} via phone number match`);
            await db.execute(sql`
              UPDATE appointments 
              SET call_id = ${callId}, updated_at = NOW()
              WHERE id = ${row.id}
    ...
    console.error(`❌ [Appointment Linker] Error linking pending appointments:`, err.message);
  }
}
...
function setupElevenLabsMessageHandlers(
  ...
  // Start initial silence timer when connection is established
  if (silenceTimerRef.current) {
    clearTimeout(silenceTimerRef.current);
  }
  silenceTimerRef.current = setTimeout(async () => {
    console.log(`🔇 [Silence Detected] No activity/speech at the start of the call for 30 seconds, ending call...`);
    await endCall('30 seconds of initial silence detected');
  }, 30000);
    ...
    try {
      ...
      switch (message.type) {
        ...
        case 'audio':
          ...
          // Reset silence timer on audio chunk to prevent timeout while agent is speaking
          if (silenceTimerRef.current) {
            clearTimeout(silenceTimerRef.current);
          }
          silenceTimerRef.current = setTimeout(async () => {
            console.log(`🔇 [Silence Detected] No activity for 30 seconds after agent audio, ending call...`);
        ...
        case 'agent_response':
          ...
          const goodbyePhrases = [
            ...
            'have a nice day', 'have a wonderful day', 'thank you for choosing',
            'take care', 'alvida', 'dhanyawad', 'shukriya', 'aapka din shubh ho'
          ];
          ...
          if (saidGoodbye && !lowerText.includes('?')) {
            console.log(`👋 [Agent Farewell] Agent said goodbye - setting short 5-second silence timeout to end call`);
            if (silenceTimerRef.current) {
              clearTimeout(silenceTimerRef.current);
            }
            silenceTimerRef.current = setTimeout(async () => {
              console.log(`👋 [Agent Farewell] Ending call 5 seconds after agent said goodbye`);
              await endCall('Agent said goodbye');
            }, 5000);
          }
          break;
...
async function initializeElevenLabsConnection(
  ...
  elevenLabsWs.on('close', async (code, reason) => {
    ...
    // Hang up Twilio call when ElevenLabs connection closes (e.g. via end_call tool or graceful close)
    await endCall('ElevenLabs connection closed');
  });
...
export async function handleElevenLabsWebhook(req: Request, res: Response) {
  try {
      ...
      if (audioConvId) {
        try {
          ...
          if (existingCall) {
            ...
            const sipCallResult = await db.execute(sql`
              SELECT * FROM sip_calls WHERE elevenlabs_conversation_id = ${audioConvId} OR external_call_id = ${audioConvId} LIMIT 1
            `);
    ...
    if (type === 'call_initiation_failure') {
      ...
      // Try to update the call record if it exists — check both calls and sip_calls tables
      if (conversation_id) {
        let callRecord = null;
        let callTable: 'calls' | 'sip_calls' = 'calls';

        // First try the main calls table
        ...
        if (mainCallRecord) {
          ...
          // Try sip_calls table
          const sipCallResult = await db.execute(sql`
            SELECT * FROM sip_calls WHERE elevenlabs_conversation_id = ${conversation_id} OR external_call_id = ${conversation_id} LIMIT 1
          `);
          if (sipCallResult.rows.length > 0) {
            callRecord = sipCallResult.rows[0] as any;
            callTable = 'sip_calls';
          }
        }

        if (callRecord) {
          if (callTable === 'sip_calls') {
            await db.execute(sql`
              UPDATE sip_calls SET 
                status = 'failed',
                ended_at = NOW(),
            ...
            `);
            console.log(`   Updated SIP call ${callRecord.id} as failed`);
          } else {
            await db
              .update(calls)
              .set({
          ...
          // No call record found yet — store as pending failure for race condition handling
          if (conversation_id && failure_reason) {
            pendingCallFailures.set(conversation_id, {
              failure_reason,
              metadata: failureMetadata,
              timestamp: Date.now(),
            });
            console.log(`   ⏳ Stored pending failure for conversation ${conversation_id} (will be applied if SIP record is created later)`);
          }
        }
      }
      await recordWebhookReceived('elevenlabs');
    ...
    // Support both standard 'post_call_transcription' and OpenTelemetry-enabled 'post_call_transcription_otel' formats
    if (type !== 'post_call_transcription' && type !== 'post_call_transcription_otel') {
    ...
    const customerPhone = data?.user_id || data?.userId || null;
    ...
    if (!callRecord && agent_id) {
      ...
      const webhookPhoneNumber = extractedToNumber || extractedFromNumber || customerPhone;
      ...
      if (webhookPhoneNumber) {
        ...
        if (agent && (agent.type === 'natural' || agent.type === 'flow')) {
          ...
          try {
            const webhookPhoneNoPlus = webhookPhoneNumber.replace(/^\+/, '');
            const webhookPhoneWithPlus = '+' + webhookPhoneNoPlus;
              ...
              .where(
                and(
                  or(
                    ...
                    eq(calls.phoneNumber, webhookPhoneNoPlus),
                    eq(calls.phoneNumber, webhookPhoneWithPlus)
                  ),
            ...
            if (pendingCallQuery.length > 0) {
              const targetDigits = webhookPhoneNumber.replace(/[^\d]/g, '');
              const matchedRow = pendingCallQuery.find(row => {
                const dbDigits = (row.call.phoneNumber || '').replace(/[^\d]/g, '');
                return dbDigits === targetDigits || dbDigits.endsWith(targetDigits) || targetDigits.endsWith(dbDigits);
              });

              if (matchedRow && matchedRow.call) {
                callRecord = matchedRow.call;
                console.log(`   ✅ Found matching pre-created batch call record: ${callRecord.id} (status: ${callRecord.status})`);
              }
            }

            if (!callRecord) {
              console.log(`   No batch call found by phone number formats for ${webhookPhoneNumber}`);
            }
            ...
            if (isTypeMismatchError(pendingQueryError)) {
              ...
              try {
                ...
                const rawResult = await db.execute(sql`
                  ...
                  WHERE (c.phone_number = ${webhookPhoneNumber} OR c.phone_number = ${webhookPhoneNoPlus})
      ...
      if (!callRecord && metadata?.batch_call?.batch_call_id) {
        ...
        const webhookPhone = extractedToNumber || extractedFromNumber || customerPhone;
        ...
        if (agent_id) {
          ...
          try {
            if (agent && agent.telephonyProvider === 'elevenlabs-sip') {
              ...
              // Fallback for OTel webhooks where metadata is flat/absent
              if (!callerPhoneNumber && !calledNumber && customerPhone) {
                if (direction === 'inbound') {
                  callerPhoneNumber = customerPhone;
                } else {
              ...
              // BATCH CAMPAIGN INTERCEPT: For SIP agents, batch campaign calls are stored in
              // the `calls` table (not sip_calls). Check for a matching batch campaign call
              // BEFORE going through the SIP call creation flow.
              try {
                const batchCallResult = await db
                  .select()
                  .from(calls)
                  .where(
                    and(
                      sql`${calls.metadata}->>'elevenLabsAgentId' = ${agent_id}`,
                      sql`${calls.metadata}->>'batchCall' = 'true'`,
                      eq(calls.callDirection, 'outgoing'),
                      isNull(calls.elevenLabsConversationId)
                    )
                  )
                ...
                if (batchCallResult.length > 0) {
                  // Try phone number matching if we have phone data
                  const webhookPhone = calledNumber || callerPhoneNumber || extractedToNumber || extractedFromNumber || customerPhone;
                  let matchedBatchCall = null;

                  if (webhookPhone && webhookPhone !== 'N/A') {
                    const targetDigits = webhookPhone.replace(/[^\d]/g, '');
                    if (targetDigits) {
                      matchedBatchCall = batchCallResult.find(c => {
                  ...
                  if (matchedBatchCall) {
                    console.log(`📞 [ElevenLabs SIP] Found matching batch campaign call: ${matchedBatchCall.id} (status: ${matchedBatchCall.status}, phone: ${matchedBatchCall.phoneNumber})`);

                    // Link conversation_id and update status
                    const updateData: Record<string, any> = {
                      elevenLabsConversationId: conversation_id,
                    };

                    // If the call was prematurely failed by expireRemainingPendingCalls, rescue it
                    // post_call_transcription webhook = call completed on ElevenLabs
                    if (matchedBatchCall.status === 'failed' || matchedBatchCall.status === 'pending') {
                      updateData.status = 'completed';
                      console.log(`   ✅ Rescuing batch call from '${matchedBatchCall.status}' → 'completed'`);
                    }

                    await db
                      .update(calls)
                      .set(updateData)
                      .where(eq(calls.id, matchedBatchCall.id));

                    // Trigger full sync for transcript, summary, recording
                    const { callSyncService } = await import('../services/call-sync');
                    callSyncService.syncCall(matchedBatchCall.id).catch(syncErr => {
                      console.error(`❌ [ElevenLabs SIP] Background sync failed for batch call ${matchedBatchCall!.id}:`, syncErr.message);
                    });

                    console.log(`✅ [ElevenLabs SIP] Linked batch call ${matchedBatchCall.id} to conversation ${conversation_id}, sync triggered`);
                    await recordWebhookReceived('elevenlabs');
                    return res.sendStatus(200);
                  } else {
                    console.log(`   [ElevenLabs SIP] No matching batch campaign call found by phone number — proceeding with SIP call flow`);
                  }
                } else {
                  console.log(`   [ElevenLabs SIP] No unlinked batch campaign calls found — proceeding with SIP call flow`);
                }
              } catch (batchInterceptErr: any) {
                console.error(`   [ElevenLabs SIP] Batch intercept error (non-fatal): ${batchInterceptErr.message}`);
                // Continue with normal SIP flow on error
              }
            ...
            `);
              ...
              if (existingSipCall) {
                ...
                const resolvedStatus = (syncedData as any).status || status;
                const isCompletedCall = resolvedStatus === 'completed' || resolvedStatus === 'done' || type === 'post_call_transcription' || type === 'post_call_transcription_otel';
                const isFailedCall = resolvedStatus === 'failed' || resolvedStatus === 'error' || resolvedStatus === 'timeout' || resolvedStatus === 'no-answer';
                const finalStatus = isCompletedCall ? 'completed' : (isFailedCall ? 'failed' : (resolvedStatus || 'in-progress'));

                const resolvedDuration = syncedData.duration || callDuration || 0;
                const resolvedFromNumber = syncedData.phoneNumber || callerPhoneNumber || null;
                const resolvedToNumber = syncedData.calledNumber || calledNumber || null;

                // Calculate proper started_at from ended_at - duration
                const endedAt = new Date();
                const startedAt = new Date(endedAt.getTime() - (resolvedDuration * 1000));
                ...
                UPDATE sip_calls SET
                  ...
                  from_number = COALESCE(${resolvedFromNumber}, from_number),
                  to_number = COALESCE(${resolvedToNumber}, to_number),
              ...
              `);
                ...
                if (isCompletedCall && resolvedDuration > 0) {
                  ...
                  `);
                    ...
                    notifyCreditFailure({
                      ...
                      creditsRequired: Math.max(1, Math.ceil(resolvedDuration / 60)),
                      currentBalance: 0,
                  ...
                  console.log(`💳 [ElevenLabs SIP] Processing credit deduction for ${resolvedDuration}s call`);
                  const creditResult = await deductSipCallCredits(existingSipCall.id, resolvedDuration, 'elevenlabs-sip');
                  if (creditResult.success || creditResult.alreadyDeducted) {
                ...
                const isOutboundSip = direction === 'outbound';
                let sipMessagePhone: string | null = null;
                if (isOutboundSip) {
                  const toNumber = resolvedToNumber || calledNumber;
                  const toDigits = (toNumber || '').replace(/[^0-9]/g, '');
                  const fromDigits = (resolvedFromNumber || callerPhoneNumber || '').replace(/[^0-9]/g, '');
                  if (toNumber && toNumber !== 'N/A' && toNumber !== 'Unknown' && toDigits.length >= 6 && toDigits !== fromDigits) {
                    sipMessagePhone = toNumber;
                  }
                } else {
                  const fromNumber = resolvedFromNumber || callerPhoneNumber;
                  if (fromNumber && fromNumber !== 'N/A' && fromNumber !== 'Unknown') {
                    sipMessagePhone = fromNumber;
                  }
                }

                if (isCompletedCall && sipMessagePhone) {
                  import('../services/post-call-messaging').then(({ triggerPostCallMessaging }) => {
                    triggerPostCallMessaging({
                      elevenLabsAgentId: agent_id,
                      userId: existingSipCall.user_id,
                      callerPhone: sipMessagePhone!,
                      callId: existingSipCall.id,
                ...
                // Trigger webhook for completed SIP call
                if (isCompletedCall && existingSipCall.user_id) {
                  triggerCallCompletedWebhook({
                    userId: existingSipCall.user_id,
                    callId: existingSipCall.id,
                    callDirection: existingSipCall.direction || direction || 'inbound',
                    duration: resolvedDuration,
                    fromNumber: resolvedFromNumber,
                    toNumber: resolvedToNumber,
                    phoneNumber: resolvedFromNumber,
                    recordingUrl: syncedData.recordingUrl || null,
                    transcript: syncedData.transcript || null,
                    aiSummary: syncedData.aiSummary || null,
                    classification: syncedData.classification || null,
                    sentiment: syncedData.sentiment || null,
                    startedAt: existingSipCall.started_at || startedAt,
                    endedAt: endedAt,
                    agentId: agent_id || existingSipCall.agent_id || null,
                    isSipCall: true,
                  }).catch(err => console.error(`❌ [Webhook] SIP completed call webhook error:`, err.message));
                }
              ...
              if (!sipPhoneNumber) {
                ...
                // FALLBACK: For batch campaign calls, call records are in the `calls` table (not sip_calls).
                // Try to match by conversation_id or metadata.elevenLabsAgentId + phone.
                // This rescues webhooks for completed batch calls that the scheduler couldn't sync.
                try {
                  // First try matching by metadata.elevenLabsAgentId on the calls table
                  const batchFallbackResult = await db
                    .select()
                    .from(calls)
                    .where(
                      and(
                        ...
                        sql`${calls.metadata}->>'batchCall' = 'true'`,
                        eq(calls.callDirection, 'outgoing'),
                        // Look for calls that haven't been fully synced yet
                        or(
                          isNull(calls.elevenLabsConversationId),
                          eq(calls.elevenLabsConversationId, conversation_id)
                        )
                      )
                    )
                    .orderBy(sql`${calls.createdAt} DESC`)
                  ...
                  if (batchFallbackResult.length > 0) {
                    // Try to match by phone number if available, otherwise take first match
                    const webhookPhone = extractedToNumber || extractedFromNumber;
                    let matchedCall = null;

                    if (webhookPhone) {
                      const targetDigits = webhookPhone.replace(/[^\d]/g, '');
                      matchedCall = batchFallbackResult.find(c => {
                        const dbDigits = (c.phoneNumber || '').replace(/[^\d]/g, '');
                        return dbDigits === targetDigits || dbDigits.endsWith(targetDigits) || targetDigits.endsWith(dbDigits);
                      });
                    }
                    ...
                    if (!matchedCall) {
                      // Take the first call without a conversation_id
                      matchedCall = batchFallbackResult.find(c => !c.elevenLabsConversationId) || batchFallbackResult[0];
                    }

                    if (matchedCall) {
                      console.log(`📞 [ElevenLabs SIP] Found batch campaign call record: ${matchedCall.id} (status: ${matchedCall.status})`);

                      // Update conversation_id so future webhooks can match
                      await db
                        .update(calls)
                        .set({ elevenLabsConversationId: conversation_id })
                        .where(eq(calls.id, matchedCall.id));

                      // Trigger a full sync to get transcript, summary, recording
                      const { callSyncService } = await import('../services/call-sync');
                      callSyncService.syncCall(matchedCall.id).catch(err => {
                        console.error(`❌ [ElevenLabs SIP] Background sync failed for batch call ${matchedCall!.id}:`, err.message);
                      });

                      console.log(`✅ [ElevenLabs SIP] Linked batch call ${matchedCall.id} to conversation ${conversation_id}, sync triggered`);
                      return res.sendStatus(200);
                    }
                  }

                  console.log(`   [ElevenLabs SIP] No batch campaign call match found either`);
                } catch (batchFallbackErr: any) {
                  console.error(`   [ElevenLabs SIP] Batch call fallback error: ${batchFallbackErr.message}`);
                }
              ...
              const resolvedDuration = syncedData.duration || callDuration || 0;
              const resolvedFromNumber = syncedData.phoneNumber || callerPhoneNumber || 'Unknown';
              const resolvedToNumber = syncedData.calledNumber || calledNumber || sipPhoneNumber.phoneNumber;
              ...
              VALUES (
                ...
                'elevenlabs-sip', ${conversation_id}, ${resolvedFromNumber}, ${resolvedToNumber},
                ${direction}, ${finalStatus}, ${resolvedDuration}, 
            ...
            `);
              ...
              // Link pending appointments created during this conversation
              await linkPendingAppointments({
                userId: agent.userId,
                callId: sipCallRecord.id,
                conversationId: conversation_id,
                phoneNumber: resolvedFromNumber || undefined,
                isSipCall: true,
              });

              // Credit balance check + deduction for completed SIP calls
              if (isCompletedCall && resolvedDuration > 0 && sipCallRecord?.id) {
                const { checkUserCreditBalance } = await import('../services/credit-service');
                ...
                console.log(`💳 [ElevenLabs SIP] Processing credit deduction for ${resolvedDuration}s call`);
                const creditResult = await deductSipCallCredits(sipCallRecord.id, resolvedDuration, 'elevenlabs-sip');
                if (creditResult.success || creditResult.alreadyDeducted) {
                  console.log(`✅ [ElevenLabs SIP] Credits deducted: ${creditResult.creditsDeducted}`);
              ...
              if (isCompletedCall && sipMessagePhone && sipCallRecord?.id) {
                import('../services/post-call-messaging').then(({ triggerPostCallMessaging }) => {
                  triggerPostCallMessaging({
                    elevenLabsAgentId: agent_id,
                    userId: agent.userId,
              ...
              // Trigger webhook for completed SIP call
              if (isCompletedCall && sipCallRecord && agent.userId) {
                triggerCallCompletedWebhook({
                  userId: agent.userId,
                  callId: sipCallRecord.id,
          ...
          try {
            ...
            `);
              ...
              `);
                  ...
                  const resolvedStatus = (syncedData as any).status || status;
                  const isCompletedCall = resolvedStatus === 'completed' || resolvedStatus === 'done' || type === 'post_call_transcription' || type === 'post_call_transcription_otel';
                  const isFailedCall = resolvedStatus === 'failed' || resolvedStatus === 'error' || resolvedStatus === 'timeout' || resolvedStatus === 'no-answer' || !!errorInfo;
                  const finalStatus = isCompletedCall ? 'completed' : (isFailedCall ? 'failed' : (resolvedStatus || 'in-progress'));

                  const resolvedDuration = syncedData.duration || callDuration || 0;
                  const resolvedFromNumber = syncedData.phoneNumber || sipFallbackFrom || null;
                  const resolvedToNumber = syncedData.calledNumber || sipFallbackTo || null;
                ...
                `);
                    ...
                    if (isCompletedCall && resolvedDuration > 0 && !errorInfo) {
                      const { checkUserCreditBalance } = await import('../services/credit-service');
                    ...
                    // Trigger post-call messaging for completed SIP calls (fire-and-forget)
                    if (isCompletedCall && agentForSip) {
                      const isOutboundSip = direction === 'outbound';
                      let sipMessagePhone: string | null = null;
                      if (isOutboundSip) {
                        const toNumber = resolvedToNumber || sipFallbackTo;
                        const toDigits = (toNumber || '').replace(/[^0-9]/g, '');
                        const fromDigits = (resolvedFromNumber || sipFallbackFrom || '').replace(/[^0-9]/g, '');
                        if (toNumber && toNumber !== 'N/A' && toNumber !== 'Unknown' && toDigits.length >= 6 && toDigits !== fromDigits) {
                          sipMessagePhone = toNumber;
                        }
                      } else {
                        const fromNumber = resolvedFromNumber || sipFallbackFrom;
                        if (fromNumber && fromNumber !== 'N/A' && fromNumber !== 'Unknown') {
                          sipMessagePhone = fromNumber;
                        }
                      }
                      ...
                      if (sipMessagePhone) {
                        import('../services/post-call-messaging').then(({ triggerPostCallMessaging }) => {
                          triggerPostCallMessaging({
                            elevenLabsAgentId: agentForSip.id,
                            userId: userId,
                            callerPhone: sipMessagePhone!,
                            callId: existingSipCall.id,
                          }).catch(err => console.error(`❌ [Post-Call Messaging] SIP update fallback error: ${err.message}`));
                        }).catch(err => console.error(`❌ [Post-Call Messaging] Import error: ${err.message}`));
                      }
                    }

                    // Trigger webhook for completed SIP call
                    if (isCompletedCall && userId) {
                      triggerCallCompletedWebhook({
                        userId,
                        callId: existingSipCall.id,
                        callDirection: direction || 'inbound',
                    ...
                    const resolvedFallbackFrom = resolvedFromNumber || 'Unknown';
                    const resolvedFallbackTo = resolvedToNumber || matchedSipPhone.phone_number;
                  ...
                  VALUES (
                    ...
                    'elevenlabs-sip', ${conversation_id}, ${resolvedFallbackFrom}, ${resolvedFallbackTo},
                    ${direction}, ${finalStatus}, ${resolvedDuration}, 
                    ${syncedData.rawTranscript ? JSON.stringify(syncedData.rawTranscript) : null}::jsonb,
                    ${syncedData.sentiment || null},
                    ${syncedData.classification || null},
                ...
                `);
                    ...
                    await linkPendingAppointments({
                      ...
                      callId: sipCallRecord.id,
                      conversationId: conversation_id,
                      phoneNumber: resolvedFallbackFrom || undefined,
                      isSipCall: true,
                    });

                    // Credit check + deduction for completed calls only
                    if (isCompletedCall && resolvedDuration > 0 && sipCallRecord?.id && !errorInfo) {
                      const { checkUserCreditBalance } = await import('../services/credit-service');
                      const creditCheck = await checkUserCreditBalance(userId);
                    ...
                    // Trigger post-call messaging for completed SIP calls (fire-and-forget)
                    if (isCompletedCall && agentForSip && sipCallRecord?.id) {
                      const isOutboundSip = direction === 'outbound';
                      let sipMessagePhone: string | null = null;
                      if (isOutboundSip) {
                        const toNumber = resolvedToNumber || sipFallbackTo;
                      ...
                      if (sipMessagePhone) {
                        import('../services/post-call-messaging').then(({ triggerPostCallMessaging }) => {
                          triggerPostCallMessaging({
                            ...
                            callerPhone: sipMessagePhone!,
                            callId: sipCallRecord.id,
                          }).catch(err => console.error(`❌ [Post-Call Messaging] SIP create fallback error: ${err.message}`));
                        }).catch(err => console.error(`❌ [Post-Call Messaging] Import error: ${err.message}`));
                      }
                    }

                    // Trigger webhook for completed SIP call
                    if (isCompletedCall && sipCallRecord && userId) {
                      triggerCallCompletedWebhook({
                        userId,
                        callId: sipCallRecord.id,
                        callDirection: direction || 'inbound',
                        duration: resolvedDuration,
                        fromNumber: resolvedFallbackFrom,
                        toNumber: resolvedFallbackTo,
                        phoneNumber: resolvedFallbackFrom,
                        recordingUrl: syncedData.recordingUrl || null,
                        transcript: syncedData.transcript || null,
                        aiSummary: syncedData.aiSummary || null,
                        classification: syncedData.classification || null,
    ...
    try {
      ...
      try {
        ...
        if (callPhone && agent_id && callRecord.userId) {
          ...
          for (const appointment of recentAppointments) {
            // Try matching by conversation ID first
            const apptConvId = appointment.metadata?.elevenLabsConversationId || appointment.metadata?.conversationId;
            const convIdMatches = apptConvId && conversation_id && apptConvId === conversation_id;
            ...
            // Agent ID match: for incoming calls where phone may be unavailable,
            // match by agent ID if there's only 1 unlinked appointment in the window
            const agentIdMatch = appointment.metadata?.elevenLabsAgentId && agent_id &&
              appointment.metadata.elevenLabsAgentId === agent_id;

            if (convIdMatches || phoneMatches || fuzzyPhoneMatch || (agentIdMatch && recentAppointments.length === 1)) {
              console.log(`📅 [ElevenLabs Webhook] Found appointment ${appointment.id} booked during call - linking and updating metadata`);
              console.log(`   Matches: convId=${!!convIdMatches}, phone=${phoneMatches}, fuzzy=${fuzzyPhoneMatch}, agentId=${agentIdMatch}`);
      ...
      // Refresh appointment tool date context for incoming agents (background, non-blocking)
      // This ensures the tool has the current date for "tomorrow" calculations on the next call
      if (callRecord.callDirection === 'incoming' && agent_id) {
        import('../services/elevenlabs').then(({ ElevenLabsService }) => {
          // Find the agent's credential to create the service
          import('../services/elevenlabs-pool').then(({ ElevenLabsPoolService }) => {
            db.select({ elevenLabsCredentialId: agents.elevenLabsCredentialId })
              .from(agents)
              .where(eq(agents.elevenLabsAgentId, agent_id))
              .limit(1)
              .then(async ([agentRow]) => {
                if (agentRow?.elevenLabsCredentialId) {
                  const credential = await ElevenLabsPoolService.getCredentialById(agentRow.elevenLabsCredentialId);
                  if (credential?.apiKey) {
                    const service = new ElevenLabsService(credential.apiKey);
                    await service.refreshAppointmentToolWithCurrentDate(agent_id);
                    console.log(`📅 [ElevenLabs Webhook] Refreshed appointment tool date for incoming agent ${agent_id}`);
                  }
                }
              }).catch(() => { });
          }).catch(() => { });
      ...
      // Trigger user webhook for completed call
      if (callRecord.userId) {
        const finalDuration = updates.duration || callRecord.duration || callDuration;
        const finalFromNumber = updates.fromNumber || callRecord.fromNumber || null;
        const finalToNumber = updates.toNumber || callRecord.toNumber || null;
        const finalPhoneNumber = updates.phoneNumber || callRecord.phoneNumber || null;
        const finalRecordingUrl = updates.recordingUrl || callRecord.recordingUrl || null;
        const finalTranscript = updates.transcript || callRecord.transcript || null;
        const finalAiSummary = updates.aiSummary || callRecord.aiSummary || null;
        const finalClassification = updates.classification || callRecord.classification || null;
        const finalSentiment = updates.sentiment || callRecord.sentiment || null;

        triggerCallCompletedWebhook({
          userId: callRecord.userId,
          callId: callRecord.id,
          callDirection: callRecord.callDirection || (callRecord.callDirection === 'incoming' ? 'inbound' : 'outbound'),
          duration: finalDuration,
          fromNumber: finalFromNumber,
          toNumber: finalToNumber,
          phoneNumber: finalPhoneNumber,
          twilioSid: updates.twilioSid || callRecord.twilioSid || null,
          recordingUrl: finalRecordingUrl,
          transcript: finalTranscript,
          aiSummary: finalAiSummary,
          classification: finalClassification,
          sentiment: finalSentiment,
          startedAt: callRecord.startedAt || null,
          endedAt: updates.endedAt || callRecord.endedAt || null,
          agentId: agent_id || callRecord.agentId || callRecord.incomingAgentId || null,
          campaignId: callRecord.campaignId || null,
          contactId: callRecord.contactId || null,
          isSipCall: false,
        }).catch(err => console.error(`❌ [Webhook] Error triggering completed call webhook:`, err.message));
      }
...
export async function fetchElevenLabsConversation(req: Request, res: Response) {
  try {
    ...
    const callDurationSecs = conversationDetails.call_duration_secs ||
      conversationDetails.metadata?.call_duration_secs ||
      conversationDetails.metadata?.phone_call?.call_duration_secs ||
      0;
    ...
    if (isSipCall) {
      ...
      await db
        .update(sipCalls)
        .set({
          durationSeconds: callDurationSecs || callRecord.durationSeconds,
      ...
      await db
        .update(calls)
        .set({
          duration: callDurationSecs || callRecord.duration,
          transcript: transcriptText || callRecord.transcript,
          aiSummary: conversationDetails.analysis?.summary || callRecord.aiSummary,
          recordingUrl: conversationDetails.recording_url || callRecord.recordingUrl,
          status: conversationDetails.status === 'done' ? 'completed' : callRecord.status,
...
export async function handleAppointmentToolWebhook(req: Request, res: Response) {
  ...
  try {
    ...
    let agent = await db
      .select({ id: agents.id, userId: agents.userId, flowId: agents.flowId, name: agents.name })
      .from(agents)
    ...
    if (agent.length === 0 && callId) {
      ...
      const sipCallByCallId = await db.execute(sql`
        SELECT sc.agent_id, a.id as agent_id_verified, a.user_id, a.flow_id, a.name as agent_name, sc.id as call_id
        FROM sip_calls sc
    ...
    const finalConvId = conversationId || conversation_id || null;
    if (finalConvId) {
      metadata.elevenLabsConversationId = finalConvId;
      console.log(`📅 [Appointment Webhook] Received ElevenLabs Conversation ID: ${finalConvId}`);
    }
    // Store the ElevenLabs agent ID for matching during appointment linking
    if (elevenLabsAgentId) {
      metadata.elevenLabsAgentId = elevenLabsAgentId;
      metadata.agentId = dbAgentId;
      metadata.agentName = agent[0]?.name;
    }
...
export async function handleRAGToolWebhook(req: Request, res: Response) {
  ...
  try {
    ...
    if (!validateRAGWebhookToken(providedToken)) {
      ...
      console.warn(`   URL token (full): ${urlToken || 'none'}`);
      console.warn(`   Header token (full): ${headerToken || 'none'}`);
      return res.status(401).json({
