    ...
    if (userId) {
      ...
      if (result.success || result.alreadyDeducted) {
        await db
          ...
          .where(eq(calls.id, callId));
        console.log(`💳 [Credit Deduction] Updated calls.creditsUsed to ${creditsToDeduct} for call ${callId}`);
      }
      return result;
    } else {
...
export async function triggerCallCompletedWebhook(params: {
  userId: string;
  callId: string;
  callDirection: 'incoming' | 'outgoing' | 'inbound' | 'outbound' | string;
  duration: number;
  ...
  try {
    const isIncoming = params.callDirection === 'incoming' || params.callDirection === 'inbound';
    const eventType = isIncoming ? 'inbound_call.completed' : 'call.completed';

    console.log(`🔔 [Webhook] Triggering ${eventType} for call ${params.callId}`);
    ...
    // Fetch campaign and contact if outbound/campaign
    ...
    // Fallback: If no contactId but we have a phone number for inbound call, look up contact
    if (!contactData && isIncoming && params.fromNumber) {
      const digits = params.fromNumber.replace(/[^0-9]/g, '');
      if (digits.length >= 6) {
        const [contact] = await db
          .select()
          .from(contacts)
    ...
    const payload: Record<string, any> = {
      call: {
        id: params.callId,
        callSid: params.twilioSid || null,
        direction: isIncoming ? 'inbound' : 'outbound',
        status: 'completed',
        duration: params.duration,
        fromNumber: params.fromNumber,
        toNumber: params.toNumber,
        phoneNumber: params.phoneNumber || params.fromNumber || params.toNumber || null,
        recordingUrl: params.recordingUrl || null,
        transcript: params.transcript || null,
        aiSummary: params.aiSummary || null,
        classification: params.classification || null,
        sentiment: params.sentiment || null,
        startedAt: params.startedAt instanceof Date ? params.startedAt.toISOString() : (params.startedAt || null),
        endedAt: params.endedAt instanceof Date ? params.endedAt.toISOString() : (params.endedAt || null),
        isSipCall: !!params.isSipCall,
      },
      agent: agentData,
      campaign: campaignData,
      contact: contactData || (params.fromNumber ? { phone: params.fromNumber } : null),
      analysis: {
        classification: params.classification || null,
        sentiment: params.sentiment || null,
        summary: params.aiSummary || null,
      }
    };

    await webhookDeliveryService.triggerEvent(params.userId, eventType, payload, params.campaignId);
    console.log(`✅ [Webhook] Successfully triggered ${eventType} for call ${params.callId}`);
  } catch (error: any) {
    console.error(`❌ [Webhook] Error triggering completed call webhook:`, error.message);
  }
}
