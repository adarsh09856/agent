    ...
    if (process.env.APPOINTMENT_WEBHOOK_SECRET) {
      ...
      try {
        if (fs.existsSync(PERSISTED_SECRET_PATH)) {
          ...
          console.log(`\u{1F4C5} [Appointment Tool] Loaded persisted webhook secret from file`);
        }
      } catch (err) {
      }
      if (!appointmentWebhookSecret) {
        appointmentWebhookSecret = crypto.randomBytes(32).toString("hex");
        console.log(`\u{1F4C5} [Appointment Tool] Generated new webhook secret`);
      }
      try {
        fs.writeFileSync(PERSISTED_SECRET_PATH, appointmentWebhookSecret, { mode: 384 });
        console.log(`\u{1F4C5} [Appointment Tool] Persisted webhook secret to file (set APPOINTMENT_WEBHOOK_SECRET env var for explicit control)`);
      } catch (err) {
        console.warn(`\u{1F4C5} [Appointment Tool] Could not persist webhook secret to file - secret will change on restart`);
      }
    }
  }
...
function getBookAppointmentWebhookTool(agentId, callId) {
    ...
    api_schema: {
      ...
      request_body_schema: {
        ...
        properties: {
          ...
          conversationId: {
            type: "string",
            description: "ALWAYS set this to the string {{conversation_id}} to track which call this appointment belongs to."
          }
        },
        required: ["contactName", "contactPhone", "appointmentDate", "appointmentTime", "conversationId"]
      }
    }
  };
}
function getAppointmentToolForAgent(elevenLabsAgentId) {
  const domain = getDomain();
  const secret = getAppointmentWebhookSecret();
...
export {
  getAppointmentToolForAgent,
  getAppointmentWebhookSecret,
  getBookAppointmentWebhookTool,
