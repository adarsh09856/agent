...
import { db } from '../db';
import { campaigns, contacts, calls, agents, phoneNumbers, plivoPhoneNumbers, sipPhoneNumbers, flowExecutions, flows, plivoCalls, twilioOpenaiCalls, sipCalls } from '../../shared/schema';
import { nanoid } from 'nanoid';
import { eq, inArray, sql, and, isNotNull, lte, not } from 'drizzle-orm';
 ...
 */
export function isNonElevenLabsBatchJob(batchJobId: string | null | undefined): boolean {
  if (!batchJobId) return true;
  return (
    batchJobId.startsWith('batch_') ||
    ...
    batchJobId.startsWith('btcal_demo_') ||
    batchJobId.startsWith('demo-')
  );
}
...
export class CampaignExecutor {
      ...
      if (campaign.agentId) {
        ...
        if (!agentResult) {
          const veAgentResult = await db.execute(sql`
            SELECT * FROM ve_voice_agents WHERE id = ${campaign.agentId} LIMIT 1
          `);
  ...
  async executeCampaign(campaignId: string): Promise<{ batchJob: BatchJob }> {
    try {
      ...
      // Block Flow Builder agents that have no campaign-capable telephony provider
      // (openai-sip is incoming-only and is blocked later with a more specific error)
      if (agent.type === 'flow' && agent.telephonyProvider && !['plivo', 'twilio_openai', 'custom-voice-engine', 'elevenlabs-sip', 'twilio', 'openai-sip'].includes(agent.telephonyProvider)) {
        throw new Error(`This flow agent uses '${agent.telephonyProvider}' which does not support multi-contact campaigns.`);
      }

      // Route to Custom Voice Engine
      if (agent.telephonyProvider === 'custom-voice-engine') {
        console.log(`📞 [Campaign Executor] Routing to Custom Voice Engine (FreeSWITCH ESL) for campaign ${campaignId}`);
        ...
        const { CveBatchCallingService } = await importPlugin('plugins/custom-voice-engine/services/cve-batch-calling.service');
        const cveBatchService = CveBatchCallingService.getInstance(campaignId);

        cveBatchService.executeCampaign(campaignId).then(async (result: any) => {
          ...
          console.error(`[Campaign Executor] Background Custom Voice Engine campaign error:`, err);
        });
      ...
      if (agent.telephonyProvider === 'plivo') {
        ...
        plivoBatchService.executeCampaign(campaignId).then(async (result: any) => {
          if (campaign.userId && result.status === 'completed') {
            const completedContacts = await db
              .select()
              .from(contacts)
      ...
      if (agent.telephonyProvider === 'twilio_openai') {
        ...
        twilioOpenAIBatchService.executeCampaign(campaignId).then(async (result: any) => {
          if (campaign.userId && result.status === 'completed') {
      ...
      if (agent.telephonyProvider === 'elevenlabs-sip') {
        ...
        if (agent.appointmentBookingEnabled || agent.flowId) {
          console.log(`📅 [Campaign Executor] Refreshing SIP agent appointment tool and prompt for current date context...`);
          try {
            ...
            await elevenLabsServiceForTools.refreshAgentPromptWithCurrentDate(agent.elevenLabsAgentId);
            console.log(`✅ [Campaign Executor] SIP agent appointment tool and prompt refreshed with current date`);
          } catch (toolRefreshError: any) {
            console.warn(`⚠️ [Campaign Executor] Failed to refresh SIP agent tools/prompt: ${toolRefreshError.message}`);
      ...
      // Check both appointmentBookingEnabled flag AND flowId (flow agents may have appointment nodes)
      if (agent.appointmentBookingEnabled || agent.flowId) {
        console.log(`📅 [Campaign Executor] Refreshing agent tools and prompt on ElevenLabs for current date context...`);
        try {
          ...
          await elevenLabsService.refreshAgentPromptWithCurrentDate(agent.elevenLabsAgentId);
          console.log(`✅ [Campaign Executor] Agent tools and prompt refreshed with current date context`);
        } catch (toolRefreshError: any) {
          console.warn(`⚠️ [Campaign Executor] Failed to refresh agent tools/prompt: ${toolRefreshError.message}`);
          // Non-fatal - continue with campaign execution
        }
      }
  ...
  async getBatchJobStatus(campaignId: string): Promise<BatchJobWithRecipients | null> {
    ...
    const effectiveBatchJobId = campaign.batchJobId || `batch_${campaign.id.slice(0, 8)}`;

    let agent: any = null;
    if (campaign.agentId) {
      const [foundAgent] = await db.select().from(agents).where(eq(agents.id, campaign.agentId)).limit(1);
      agent = foundAgent;
    ...
    // Non-ElevenLabs campaigns (Plivo, Twilio-OpenAI, SIP) use local batch job IDs
    ...
    if (isNonElevenLabsBatchJob(campaign.batchJobId) || (agent && agent.telephonyProvider === 'openai-sip')) {
      ...
      if (campaign.batchJobId && campaign.batchJobId.startsWith('plivo-')) {
        campaignCalls = await db.select().from(plivoCalls).where(eq(plivoCalls.campaignId, campaignId));
      } else if (campaign.batchJobId && (campaign.batchJobId.startsWith('twilio_openai-') || campaign.batchJobId.startsWith('twilio-openai-'))) {
    ...
    try {
      const credential = await ElevenLabsPoolService.getCredentialForAgent(agent.id);
      if (!credential) {
        throw new Error('No ElevenLabs credential available');
      }

      const batchService = new BatchCallingService(credential.apiKey);
      const batchJob = await batchService.getBatch(campaign.batchJobId as string);
      ...
      console.warn(`[BatchCalling] Could not fetch batch job status from ElevenLabs API (${err.message}), falling back to local synthesis`);
      // Fallback: construct batch job response using local DB call records
  ...
  async cancelCampaign(campaignId: string): Promise<BatchJob | null> {
    ...
    if (agent.telephonyProvider === 'twilio_openai') {
      ...
      const campaignCalls = await db.select().from(twilioOpenaiCalls).where(eq(twilioOpenaiCalls.campaignId, campaignId));
      const completedCallsCount = campaignCalls.filter(c =>
        ['completed', 'failed', 'busy', 'no-answer'].includes(c.status)
      ).length;
      const successfulCallsCount = campaignCalls.filter(c => c.status === 'completed').length;
  ...
  async resumeCampaign(campaignId: string, reason: 'manual' | 'scheduled' = 'manual'): Promise<BatchJob | null> {
    ...
    if (agent.telephonyProvider === 'custom-voice-engine') {
      ...
      if (cveBatchService.isRunning() === false) {
        cveBatchService.resume();
      }
      ...
      cveBatchService.executeCampaign(campaignId).then(async (result: any) => {
        ...
        console.error(`❌ [CVE Campaign] Background execution error for ${campaignId}:`, err);
      });

      console.log(`▶️ [Campaign Executor] Resumed CVE campaign ${campaignId} (reason: ${reason})`);
    ...
    if (agent.telephonyProvider === 'plivo') {
      ...
      // Re-execute the campaign in background (it will pick up pending contacts)
      plivoBatchService.executeCampaign(campaignId).then(async (result) => {
        ...
        console.error(`❌ [Plivo Campaign] Background execution error for ${campaignId}:`, err);
      });
    ...
    if (agent.telephonyProvider === 'twilio_openai') {
      ...
      twilioOpenAIBatchService.executeCampaign(campaignId).then(async (result) => {
        ...
        console.error(`❌ [Twilio Campaign] Background execution error for ${campaignId}:`, err);
      });

      console.log(`▶️ [Campaign Executor] Resumed Twilio-OpenAI campaign ${campaignId} (reason: ${reason})`);

      return {
        id: `twilio-${campaignId}`,
        name: campaign.name,
        agent_id: agent.id,
        agent_name: agent.name,
        created_at_unix: Math.floor(Date.now() / 1000),
  ...
  async resumeWithNewBatch(campaignId: string): Promise<{ batchJob: BatchJob | null; contactsToCall: number }> {
    ...
    // Phone number check moved down after agent fetch
    ...
    // Check for phone number unless the agent is custom-voice-engine
    if (agent.telephonyProvider !== 'custom-voice-engine') {
      if (!campaign.phoneNumberId && !campaign.plivoPhoneNumberId && !(campaign as any).sipPhoneNumberId) {
        throw new Error('Campaign has no phone number configured');
      }
    ...
    if (agent.telephonyProvider === 'custom-voice-engine') {
      ...
      cveBatchService.executeCampaign(campaignId).then(async (result: any) => {
        if (result.completedCalls + result.failedCalls === result.totalCalls) {
          await db
            .update(campaigns)
            .set({
              status: 'completed',
        ...
        console.error(`[Campaign Executor] Error executing CVE campaign ${campaignId}: ${err.message}`);
      });

      console.log(`▶️ [Campaign Executor] Resumed CVE campaign with new batch: ${campaignId}`);

      // Need to count pending contacts to return
    ...
    if (agent.telephonyProvider === 'plivo') {
      ...
      plivoBatchService.executeCampaign(campaignId).catch(err => {
        console.error(`❌ [Plivo Campaign] Background execution error for ${campaignId}:`, err);
      });
    ...
    if (agent.telephonyProvider === 'twilio_openai') {
      ...
      twilioOpenAIBatchService.executeCampaign(campaignId).catch(err => {
        console.error(`❌ [Twilio Campaign] Background execution error for ${campaignId}:`, err);
      });

      console.log(`▶️ [Campaign Executor] Resumed Twilio-OpenAI campaign with new batch: ${campaignId}`);
    ...
    if (agent.telephonyProvider === 'elevenlabs-sip') {
      ...
      // Check both appointmentBookingEnabled flag AND flowId (flow agents may have appointment nodes)
      if (agent.appointmentBookingEnabled || agent.flowId) {
        console.log(`📅 [Campaign Executor] Refreshing SIP agent appointment tool and prompt for current date context (resume)...`);
        try {
          const elevenLabsServiceForTools = new ElevenLabsService(credential.apiKey);
          await elevenLabsServiceForTools.refreshAppointmentToolWithCurrentDate(agent.elevenLabsAgentId);
          await elevenLabsServiceForTools.refreshAgentPromptWithCurrentDate(agent.elevenLabsAgentId);
