...
import { batchInsertContacts, CONTACT_INSERT_RETAIN_RESULTS_THRESHOLD } from "../utils/batch-utils.js";
...
class ContactUploadService {
  ...
  parseContactRow(row, campaignId) {
    ...
    const phone = sciNotationPattern.test(rawPhone.trim()) ? Math.round(parseFloat(rawPhone)).toString() : rawPhone;
  ...
  async createContactsForCampaign(campaignId, contacts, currentTotalContacts) {
    ...
    const insertContacts = contacts.filter((c) => ContactUploadService.isDialablePhone(String(c.phone || ""))).map((contact) => ({
