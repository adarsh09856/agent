'use strict';
import { db } from '../db';
import { leads, leadStages, contacts, plivoCalls, twilioOpenaiCalls, campaigns, incomingConnections, agents, determineAICategory } from '@shared/schema';
  ...
  static async createLeadFromCall(callData: CallData): Promise<{ leadId: string } | null> {
    try {
      ...
      if (existingLead.length > 0) {
        ...
        updateData.aiCategory = determineAICategory({
          hasAppointment: updateData.hasAppointment !== undefined ? updateData.hasAppointment as boolean : lead.hasAppointment,
          hasFormSubmission: updateData.hasFormSubmission !== undefined ? updateData.hasFormSubmission as boolean : lead.hasFormSubmission,
          hasTransfer: updateData.hasTransfer !== undefined ? updateData.hasTransfer as boolean : lead.hasTransfer,
          hasCallback: updateData.hasCallback !== undefined ? updateData.hasCallback as boolean : lead.hasCallback,
          callbackScheduled: lead.callbackScheduled,
          leadScore: lead.leadScore,
          sentiment: (updateData.sentiment as string) || lead.sentiment,
        });
      ...
      leadData.aiCategory = determineAICategory({
        hasAppointment: leadData.hasAppointment as boolean,
        hasFormSubmission: leadData.hasFormSubmission as boolean,
        hasTransfer: leadData.hasTransfer as boolean,
        hasCallback: leadData.hasCallback as boolean,
        callbackScheduled: null,
        leadScore: 50,
        sentiment: leadData.sentiment as string | null,
      });
