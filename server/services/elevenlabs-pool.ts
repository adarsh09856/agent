...
export class ElevenLabsPoolService {
  ...
  static async syncExistingAgents(): Promise<{ synced: number; reassigned: number; rebalanced: number; total: number; credentialId: string | null }> {
    ...
    // Fetch default user ID (first admin, fallback to first user in system)
    const [adminUser] = await db
      .select()
      .from(users)
      .where(eq(users.role, "admin"))
      .limit(1);
    const defaultUserId = adminUser?.id || (await db.select({ id: users.id }).from(users).limit(1))[0]?.id;

    // Pull agents from ElevenLabs API for each active credential and import new ones
    for (const credential of activeCredentials) {
      try {
        ...
        if (response.ok) {
          const data = (await response.json()) as { agents?: Array<{ agent_id: string; name: string }> };
          if (data && Array.isArray(data.agents)) {
            for (const elAgent of data.agents) {
              const [existingAgent] = await db
                .select()
                .from(agents)
                .where(eq(agents.elevenLabsAgentId, elAgent.agent_id))
                .limit(1);
              const [existingIncomingAgent] = await db
                .select()
                .from(incomingAgents)
                .where(eq(incomingAgents.elevenLabsAgentId, elAgent.agent_id))
                .limit(1);

              if (!existingAgent && !existingIncomingAgent && defaultUserId) {
                // Fetch full details of the agent from ElevenLabs
                let details: any = null;
                try {
                  const detailResponse = await fetch(`https://api.elevenlabs.io/v1/convai/agents/${elAgent.agent_id}`, {
                    headers: {
                      "xi-api-key": credential.apiKey,
                      ...getCorrelationHeaders(),
                    },
                  ...
                  console.error(`Failed to fetch details for agent ${elAgent.agent_id}:`, detailError);
                }

                const agentConfig = details?.conversation_config?.agent;
                const tts = details?.conversation_config?.tts;
                const conversation = details?.conversation_config?.conversation;

                // Extract fields
                const systemPrompt = agentConfig?.prompt?.prompt || "";
                const firstMessage = agentConfig?.first_message || "Hello! How can I help you today?";
                const language = agentConfig?.language || "en";
                const llmModel = agentConfig?.prompt?.llm || "gpt-4o-mini";
                const temperature = agentConfig?.prompt?.temperature !== undefined ? Number(agentConfig.prompt.temperature) : 0.5;
                const elevenLabsVoiceId = tts?.voice_id || null;
                const maxDurationSeconds = conversation?.max_duration_seconds !== undefined ? Number(conversation.max_duration_seconds) : 600;
                const turnTimeout = conversation?.turn?.turn_timeout !== undefined ? Number(conversation.turn.turn_timeout) : 1.5;

                // voice settings (stability, similarity_boost, speed)
                const stability = tts?.voice_settings?.stability !== undefined ? Number(tts.voice_settings.stability) : 0.65;
                const similarityBoost = tts?.voice_settings?.similarity_boost !== undefined ? Number(tts.voice_settings.similarity_boost) : 0.85;
                const speed = tts?.voice_settings?.speed !== undefined ? Number(tts.voice_settings.speed) : 0.92;

                // Check if agent has a workflow (making it a flow agent)
                const hasWorkflow = !!details?.workflow || (details?.workflow?.nodes && Object.keys(details.workflow.nodes).length > 0);
                const type = hasWorkflow ? "flow" : "incoming";

                await db.insert(agents).values({
                  userId: defaultUserId,
                  elevenLabsCredentialId: null, // Let balancing assign it and count it as synced
                  name: elAgent.name || "Unnamed ElevenLabs Agent",
                ...
                console.log(`[ElevenLabs Pool] Imported agent ${elAgent.name} (${elAgent.agent_id}) with full details`);
              }
            }
          }
        }
      } catch (fetchError: any) {
        console.error(`Failed to fetch agents for credential ${credential.name}:`, fetchError);
      }
    }
