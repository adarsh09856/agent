...
import { sanitizeLlmModel } from '../utils/llm-sanitize';
...
interface CreateAgentParams {
  ...
  maxDurationSeconds?: number;  // Maximum duration in seconds
...
export class ElevenLabsService {
  ...
  static sanitizeLlmModel(modelId: string | undefined): string {
    return sanitizeLlmModel(modelId);
  }
  ...
  private buildSystemTools(agentConfig: {
    ...
    if (agentConfig.endConversationEnabled || agentConfig.appointmentBookingEnabled) {
      systemTools.push({
        type: "system",
        name: "end_call",
  ...
  private enhanceSystemPromptWithTools(
    ...
    if (agentConfig.hasKnowledgeBase) {
      const agentIdSuffix = agentConfig.elevenLabsAgentId ? agentConfig.elevenLabsAgentId.slice(-8) : '';
      const toolName = agentIdSuffix ? `ask_knowledge_${agentIdSuffix}` : 'ask_knowledge';

      toolInstructions.push(
        `⚠️ CRITICAL KNOWLEDGE BASE INSTRUCTION ⚠️
You have access to a knowledge base tool called "${toolName}".

MANDATORY BEHAVIOR:
- When the user asks ANY question that might require information from the knowledge base, you MUST call the "${toolName}" tool IMMEDIATELY.
...
CORRECT BEHAVIOR: User asks "What are your pricing plans?" → CALL ${toolName} tool with query "pricing plans" → Wait for response → Answer based on tool response.
...
4. Confirm the booking details with the caller after the tool responds (e.g., "Your appointment is confirmed. Thank you and goodbye!").
5. IMMEDIATELY call the end_call tool to disconnect the call.
...
IMPORTANT: Do NOT ask the caller for their phone number - you already have it from the call.`
    ...
    toolInstructions.push(
      `⚠️ CALL TERMINATION & SILENCE RULES ⚠️
1. If the user is silent for a prolonged period, ask them: "Are you there?"
2. If they still do not respond after you ask, say exactly: "I will disconnect the call since you are not responding. Goodbye."
3. Immediately after saying that, you MUST call the "end_call" tool to disconnect.`
    );
    ...
    const requestBody: any = {
      ...
      conversation_config: {
        ...
        conversation: {
          ...
          turn: {
            mode: "turn_v3",
            turn_timeout: params.turnTimeout ?? 8,
          },
        },
      },
    };
...
You are a script reader, not a conversational AI. Execute the workflow mechanically.`;
    ...
    // NOTE: end_call tool REMOVED for flow agents.
    // Flow agents use workflow end nodes to terminate calls.
    // Including end_call lets the LLM prematurely hang up after the greeting.
  ...
  async updateFlowAgentWorkflow(agentId: string, workflow: {
    ...
    // Flow agents need a longer turn_timeout (8s) since the caller needs time to answer and respond
    updatePayload.conversation_config.conversation = {
      ...(maxDurationSeconds !== undefined && { max_duration_seconds: maxDurationSeconds }),
      turn: {
        mode: "turn_v3",
        turn_timeout: additionalOptions?.turnTimeout ?? 8,
      },
    };
    ...
    // CRITICAL: first_message MUST be set — it's what triggers ElevenLabs to auto-speak
    // when the call connects. Without it, the agent waits for user input and the call
    // times out on outbound calls.
    if (firstMessage) {
      if (!updatePayload.conversation_config) {
        updatePayload.conversation_config = {};
      }
    ...
    // Add base system prompt — CRITICAL: Without this, the agent has zero instructions
    // and will not know how to handle the conversation.
    // If no basePrompt is provided, fetch the existing agent's prompt to preserve it.
    if (additionalOptions?.basePrompt) {
      ...
      // No basePrompt provided — fetch the existing agent's prompt to prevent
      // ElevenLabs from wiping it out when we PATCH tools/ignore_default_personality
      try {
        const existingAgent = await this.getAgent(agentId);
        const existingPrompt = (existingAgent as any)?.conversation_config?.agent?.prompt?.prompt;
        if (existingPrompt) {
          updatePayload.conversation_config.agent.prompt.prompt = existingPrompt;
          console.log(`   ✓ Preserved existing base prompt (${existingPrompt.length} chars)`);
        } else {
          // Fallback: set a minimal scripted agent prompt
          const fallbackPrompt = `You are a SCRIPTED phone agent. You MUST follow the workflow EXACTLY. When a workflow step contains text to say, output ONLY that text. After speaking, STOP and wait for the user's response. Do not improvise or add extra words.`;
          updatePayload.conversation_config.agent.prompt.prompt = fallbackPrompt;
          console.log(`   ⚠️ No existing prompt found — set fallback scripted prompt`);
        }
      } catch (e: any) {
        console.log(`   ⚠️ Could not fetch existing agent for prompt preservation: ${e.message}`);
      }
    }
  ...
  async refreshAppointmentToolWithCurrentDate(agentId: string): Promise<void> {
    ...
    // 1. Get/create/update the workspace tool
    const freshAppointmentTool = getAppointmentToolForAgent(agentId);
    const toolId = await this.getOrCreateWorkspaceTool(freshAppointmentTool);
    console.log(`   Workspace tool ID for appointment: ${toolId}`);

    // 2. Fetch existing agent configuration
    const existingAgent = await this.getAgent(agentId);
    const existingTools = (existingAgent as any)?.conversation_config?.agent?.prompt?.tools || [];
    const existingToolIds = (existingAgent as any)?.conversation_config?.agent?.prompt?.tool_ids || [];

    // 3. Cleanup legacy inline appointment tools from prompt.tools if any
    const nonAppointmentTools = existingTools.filter((tool: any) => {
      return !tool.name?.startsWith('book_appointment_');
    });

    if (nonAppointmentTools.length !== existingTools.length) {
      console.log(`   🧹 Cleaning up ${existingTools.length - nonAppointmentTools.length} legacy inline appointment tool(s) from prompt.tools`);
      await this.request(`/convai/agents/${agentId}`, {
        ...
        body: JSON.stringify({
          conversation_config: {
            agent: {
              prompt: {
                tools: nonAppointmentTools
              }
            }
          }
        }),
      });
      console.log(`   Legacy inline tools removed`);
    }

    // 4. Ensure the workspace tool ID is attached to the agent's tool_ids
    if (!existingToolIds.includes(toolId)) {
      console.log(`   Attaching workspace tool ID ${toolId} to agent ${agentId}`);
      ...
      console.log(`   ✅ Appointment tool attached to agent tool_ids`);
    } else {
      console.log(`   ✅ Appointment tool ${toolId} is already attached to agent tool_ids`);
    }

    // POST-REFRESH VALIDATION: Verify the workspace tool was updated with today's date
    try {
      const workspaceTools = await this.listWorkspaceTools();
      const appointmentTool = workspaceTools.find((t: any) => t.id === toolId);

      if (appointmentTool && appointmentTool.tool_config?.description) {
        const yearStr = new Date().getFullYear().toString();
        const hasCurrentYear = appointmentTool.tool_config.description.includes(yearStr);
        if (hasCurrentYear) {
          console.log(`✅ [ElevenLabs] Workspace tool verified - description includes ${yearStr}`);
        } else {
          console.warn(`⚠️ [ElevenLabs] Workspace tool description may be stale - current year ${yearStr} not found`);
        }
      }
    ...
    console.log(`✅ [ElevenLabs] Appointment tool refreshed with current date context`);
  }

  async refreshAgentPromptWithCurrentDate(agentId: string): Promise<void> {
    try {
      const now = new Date();
      const currentDateStr = now.toLocaleDateString('en-US', {
        weekday: 'long',
      ...
      console.log(`📅 [ElevenLabs] Refreshing agent prompt with current date context for agent: ${agentId}`);
      console.log(`   Today's date: ${currentDateStr}`);

      const existingAgent = await this.getAgent(agentId);
      const prompt = (existingAgent as any).conversation_config?.agent?.prompt?.prompt || '';
      if (!prompt) return;

      // Remove any existing dynamic date info blocks to avoid duplicate date blocks piling up
      let cleanPrompt = prompt.replace(/\n\n\[System Info: Today's date is [\s\S]*?\]/g, '').trim();

      // Append the fresh date info
      const updatedPrompt = `${cleanPrompt}\n\n[System Info: Today's date is ${currentDateStr}. All relative date/time calculations or discussions must align with this today's date.]`;
      ...
      console.log(`📅 [ElevenLabs] Refreshed agent prompt with current date context: ${currentDateStr}`);
    } catch (err: any) {
      console.warn(`⚠️ [ElevenLabs] Failed to refresh agent prompt with current date: ${err.message}`);
    }
  }
  ...
  async updateAgent(agentId: string, params: Partial<CreateAgentParams>): Promise<ElevenLabsAgent> {
    ...
    if (params.prompt || params.model || params.temperature !== undefined || params.voice_tone || params.personality || params.knowledge_bases !== undefined || hasAnyToolConfig) {
      ...
      // Note: Appointment booking tool is handled separately via prompt.tool_ids
      // after the main PATCH request to avoid API errors
    ...
    // Separately associate/disassociate the appointment booking tool via tool_ids
    if (params.appointmentBookingEnabled !== undefined) {
      console.log(`📅 [ElevenLabs] Updating appointment booking tool association (enabled: ${params.appointmentBookingEnabled})`);
      try {
        const appointmentTool = getAppointmentToolForAgent(agentId);

        // Fetch existing agent configuration to get current tool_ids
        const freshAgent = await this.getAgent(agentId);
        const existingToolIds: string[] = (freshAgent as any)?.conversation_config?.agent?.prompt?.tool_ids || [];

        let updatedToolIds = [...existingToolIds];

        if (params.appointmentBookingEnabled) {
          const toolId = await this.getOrCreateWorkspaceTool(appointmentTool);
          updatedToolIds = Array.from(new Set([...updatedToolIds, toolId]));
        } else {
          // Find the tool ID of the appointment tool to remove it
          const agentIdSuffix = agentId.slice(-8);
          const toolName = `book_appointment_${agentIdSuffix}`;

          const workspaceTools = await this.listWorkspaceTools();
          const targetTool = workspaceTools.find(t => t.tool_config?.name === toolName);

          if (targetTool) {
            updatedToolIds = updatedToolIds.filter(id => id !== targetTool.id);
            console.log(`   Removing tool ID ${targetTool.id} from agent ${agentId}`);
          }
        }

        // Only update if tool_ids changed
        const hasChanged = JSON.stringify(existingToolIds.sort()) !== JSON.stringify(updatedToolIds.sort());
        if (hasChanged) {
          console.log(`   Updating agent ${agentId} tool_ids to:`, updatedToolIds);
          await this.request(`/convai/agents/${agentId}`, {
            method: "PATCH",
            body: JSON.stringify({
              conversation_config: {
          ...
          console.log(`✅ Agent tool_ids updated successfully`);
        } else {
          console.log(`✅ Agent tool_ids already up to date`);
        }
      } catch (toolUpdateError: any) {
        console.error(`❌ Failed to update agent appointment tool association:`, toolUpdateError.message);
      }
    }
   ...
   */
  async updateWorkspaceTool(toolId: string, toolConfig: {
    type: "webhook";
    name: string;
    description: string;
    api_schema: {
    ...
    console.log(`🔧 Updating workspace tool: ${toolId} (${toolConfig.name})`);
    const response = await this.request<ElevenLabsWorkspaceTool>(`/convai/tools/${toolId}`, {
      method: 'PATCH',
      body: JSON.stringify({ tool_config: toolConfig }),
    });
    console.log(`✅ Workspace tool updated: ${toolId}`);
    return response;
  }
  ...
  async getOrCreateWorkspaceTool(toolConfig: {
    ...
    const checkAndUpdateIfNeeded = async (tool: ElevenLabsWorkspaceTool) => {
      const needsUpdate =
        tool.tool_config?.description !== toolConfig.description ||
        tool.tool_config?.api_schema?.url !== toolConfig.api_schema.url ||
        JSON.stringify(tool.tool_config?.api_schema?.headers) !== JSON.stringify(toolConfig.api_schema.headers) ||
        JSON.stringify(tool.tool_config?.api_schema?.request_body_schema) !== JSON.stringify(toolConfig.api_schema.request_body_schema);

      if (needsUpdate) {
        console.log(`🔧 Workspace tool ${tool.id} config is outdated. Updating...`);
        try {
          await this.updateWorkspaceTool(tool.id, toolConfig);
        } catch (updateErr: any) {
          console.warn(`⚠️ Failed to update workspace tool ${tool.id} (non-fatal):`, updateErr.message);
        }
      }
    };
    ...
    if (cachedId) {
      const existingTool = existingTools.find(t => t.id === cachedId);
      if (existingTool) {
        console.log(`🔧 Using cached workspace tool ID (validated): ${cachedId}`);
        await checkAndUpdateIfNeeded(existingTool);
        return cachedId;
      }
    ...
    const existingTool = existingTools.find(t => t.tool_config?.name === toolConfig.name);
  ...
  /**
   * Ensure ElevenLabs workspace has a post-call webhook configured.
   * Without this, post_call_transcription events never reach our server,
   * and call statuses stay stuck at "in-progress".
   */
  async ensurePostCallWebhook(): Promise<void> {
    try {
      const domain = getDomain();
      const webhookUrl = `${domain}/api/webhooks/elevenlabs`;

      // Check current workspace settings
      const settingsResponse = await fetch(`${ELEVENLABS_V1_BASE_URL}/convai/settings`, {
        headers: { 'xi-api-key': this.apiKey, 'Content-Type': 'application/json' },
      });

      if (!settingsResponse.ok) {
        console.log(`   ⚠️ Could not fetch ElevenLabs workspace settings: ${settingsResponse.status}`);
        return;
      }
      ...
      // Check if a webhook is already configured with our URL
      if (existingWebhooks?.post_call_webhook_id) {
        console.log(`   ✓ ElevenLabs post-call webhook already configured: ${existingWebhooks.post_call_webhook_id}`);
        return;
      }

      console.log(`   🔧 Configuring ElevenLabs post-call webhook → ${webhookUrl}`);

      // Create webhook via ElevenLabs API
      const createResponse = await fetch(`${ELEVENLABS_V1_BASE_URL}/webhooks`, {
        method: 'POST',
        headers: { 'xi-api-key': this.apiKey, 'Content-Type': 'application/json' },
        body: JSON.stringify({
          url: webhookUrl,
          name: 'AgentLabs Post-Call Webhook',
        }),
      });

      if (!createResponse.ok) {
        const errText = await createResponse.text();
        console.log(`   ⚠️ Failed to create ElevenLabs webhook: ${createResponse.status} - ${errText}`);
        return;
      }

      const webhookData = await createResponse.json() as any;
      const webhookId = webhookData.webhook_id;
      console.log(`   ✅ Created ElevenLabs webhook: ${webhookId}`);

      // Configure workspace to use this webhook for post-call events
      const patchResponse = await fetch(`${ELEVENLABS_V1_BASE_URL}/convai/settings`, {
        method: 'PATCH',
        headers: { 'xi-api-key': this.apiKey, 'Content-Type': 'application/json' },
        body: JSON.stringify({
          webhooks: {
      ...
      if (patchResponse.ok) {
        console.log(`   ✅ ElevenLabs workspace webhook configured successfully`);
      } else {
        const errText = await patchResponse.text();
        console.log(`   ⚠️ Failed to configure workspace webhook: ${patchResponse.status} - ${errText}`);
      }
    } catch (error: any) {
      console.log(`   ⚠️ Could not configure ElevenLabs post-call webhook: ${error.message}`);
    }
  }
