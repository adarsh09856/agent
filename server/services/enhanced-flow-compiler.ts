...
export class EnhancedFlowCompiler {
  ...
  private hasRagTool = false;

  constructor(nodes: FlowNode[], edges: FlowEdge[], options?: { knowledgeBases?: any[], hasRagTool?: boolean }) {
    this.nodes = nodes;
    this.edges = edges;
    this.knowledgeBases = options?.knowledgeBases || [];
    this.hasRagTool = options?.hasRagTool || false;
  }
  ...
  private createMessagePrompt(text: string, nodeLabel: string): string {
    if (text.includes('{{')) {
      const contactVars = ['first_name', 'last_name', 'phone', 'email', 'contact_name', 'caller_id', 'system__caller_id'];
      const hasCustomVars = Array.from(text.matchAll(/\{\{([^}]+)\}\}/g)).some(match => {
        const varName = match[1].trim();
        return !contactVars.includes(varName);
      });
      if (hasCustomVars) {
...
2. If you do not know the value for any of these variables, have a conversation with the caller to collect them.
...
4. Once the message is delivered, you may proceed to follow any additional instructions provided below (such as calling a tool).`;
    ...
    if (this.hasRagTool || this.knowledgeBases.length > 0) {
      return `### PROTOCOL: Say "${text}". Then STOP and wait for response. If the user asks a question not covered by the script, use the ask_knowledge tool or search your knowledge base to answer briefly and helpfully. After answering, ask the user if they would like to proceed with the conversation.`;
    }
  ...
  private createQuestionPrompt(question: string, variableName: string, nodeLabel: string): string {
    if (this.hasRagTool || this.knowledgeBases.length > 0) {
      return `Say exactly: '${question}' Then stop speaking and wait for response. If the user asks an unrelated question or seeks clarification not defined in the flow, use the ask_knowledge tool or search your knowledge base to answer briefly. After answering, gently remind them of the question: '${question}'.`;
    }
...
AFTER BOOKING:
When the tool returns success, say exactly: "Your appointment has been booked successfully. Thank you and goodbye!"
This phrase signals that you should proceed to the next step, and you MUST IMMEDIATELY call the end_call tool to disconnect the call.
...
IMPORTANT: Do NOT repeat your initial greeting. Have a natural conversation to collect the required information.`;
   ...
   */
  private createFormPrompt(introMessage: string, config: any, nodeLabel: string, toolName = 'submit_form'): string {
    ...
    if (fields.length > 0) {
      return generateFormCollectionPrompt(introMessage, formName, fields, toolName);
    }
...
Once all information is collected, use the ${toolName} tool to save the responses.
...
Do not transition until you have confirmed it is sent.`;
  ...
  private compileNode(node: FlowNode): ElevenLabsNode | null {
    ...
    switch (nodeType) {
      ...
      case 'message': {
        ...
        const nodeToolIds = [...additional_tool_ids];
        if (this.hasRagTool && !nodeToolIds.includes('ask_knowledge')) {
          nodeToolIds.push('ask_knowledge');
        }
        ...
        return {
          ...
          additional_tool_ids: nodeToolIds,
          additional_knowledge_base: this.knowledgeBases.map(kb => ({
            type: kb.type === 'text' ? 'text' : 'file',
            name: kb.title || kb.name,
            id: kb.elevenLabsDocId || kb.id
          })),
          conversation_config: {
            turn_taking: {
      ...
      case 'collect_info': {
        ...
        const nodeToolIds = formId ? [submitFormToolId] : [];
        if (this.hasRagTool && !nodeToolIds.includes('ask_knowledge')) {
          nodeToolIds.push('ask_knowledge');
        }
        ...
        return {
          ...
          label: label || 'Form',
          override_prompt: true,  // CRITICAL: Locks agent to exact script
          additional_prompt: this.createFormPrompt(introMessage, config, label, submitFormToolId),
          additional_tool_ids: nodeToolIds,
          additional_knowledge_base: this.knowledgeBases.map(kb => ({
            type: kb.type === 'text' ? 'text' : 'file',
            name: kb.title || kb.name,
      ...
      case 'send_whatsapp': {
        ...
        const toolId = `send_whatsapp_${node.id.slice(-8)}`; // Prefix with send_whatsapp_ so the flow router identifies it correctly
        this.toolIds.push(toolId);
        this.hasWebhookNodes = true;
  ...
  private getWaitForResponseSetting(sourceType: string, config: Record<string, any>): boolean {
    ...
    if (sourceType === 'message' || sourceType === 'greeting') {
      ...
      if (messageText.includes('{{')) {
        const contactVars = ['first_name', 'last_name', 'phone', 'email', 'contact_name', 'caller_id', 'system__caller_id'];
        const hasCustomVars = [...messageText.matchAll(/\{\{([^}]+)\}\}/g)].some(match => {
          const varName = match[1].trim();
          return !contactVars.includes(varName);
        });
        if (hasCustomVars) {
      ...
      // Respect explicit user setting if defined, otherwise default to false for messages
      return config.waitForResponse === true;
    }
  ...
  private buildWaitConditionForNodeType(sourceType: string, config: Record<string, any>): ForwardCondition {
    switch (sourceType) {
      ...
      case 'greeting':
        ...
        const messageText = config.message || config.text || '';
        if (messageText) {
          // If message has variables, check if any of them are custom variables (needs collection)
          if (messageText.includes('{{')) {
            const contactVars = ['first_name', 'last_name', 'phone', 'email', 'contact_name', 'caller_id', 'system__caller_id'];
            const hasCustomVars = [...messageText.matchAll(/\{\{([^}]+)\}\}/g)].some(match => {
              const varName = match[1].trim();
  ...
  validate(): { valid: boolean; errors: string[]; warnings: string[] } {
    ...
    // Check that all phone_number nodes have valid non-placeholder transfer numbers
    for (const [nodeId, node] of Object.entries(this.compiledNodes)) {
      if (node.type === 'phone_number') {
        const phoneNode = node as any;
        const number = phoneNode.transfer_destination?.phone_number || '';
        const cleanNumber = number.replace(/\D/g, '');
        const placeholders = ['1234567890', '1234567891', '1234567892', '12345678901', '12345678902'];
        if (!number || number.trim() === '' || placeholders.includes(cleanNumber) || cleanNumber.length < 10) {
          warnings.push(`Transfer node "${phoneNode.label || nodeId}" has an invalid or placeholder transfer phone number: "${number}". Please configure a real phone number in the builder before calling/saving.`);
        }
      }
    }
