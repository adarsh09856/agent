...
export class FlowAgentService {
   ...
   */
  static compileFlow(flowNodes: any[], flowEdges: any[], options?: { knowledgeBases?: any[], hasRagTool?: boolean }): CompileResult {
    ...
    if (USE_ENHANCED_COMPILER) {
      ...
      const compiler = new EnhancedFlowCompiler(flowNodes, flowEdges, options);
   ...
   */
  static async fetchAndCompileFlow(flowId: string, knowledgeBaseIds?: string[]): Promise<CompileResult & { flow: any }> {
    const [flow] = await db
      .select()
      .from(flows)
    ...
    // Fetch KB objects if provided
    const knowledgeBases = knowledgeBaseIds ? await this.fetchKnowledgeBases(knowledgeBaseIds) : [];
    const hasRagTool = knowledgeBases.length > 0 && isRAGEnabled();

    const compileResult = this.compileFlow(flowNodes, flowEdges, { knowledgeBases, hasRagTool });
  ...
  static async createInElevenLabs(params: FlowAgentCreateParams): Promise<{ elevenLabsAgentId: string; credentialId: string }> {
    ...
    const { workflow, firstMessage: flowFirstMessage, hasAppointmentNodes, hasFormNodes, formNodes, hasWebhookNodes, webhookNodes, hasPlayAudioNodes, playAudioNodes } = await this.fetchAndCompileFlow(params.flowId, params.knowledgeBaseIds);
  ...
  static async updateInElevenLabs(
    ...
    // Fetch and compile the flow
    const { workflow, firstMessage: flowFirstMessage, hasAppointmentNodes, hasFormNodes, formNodes, hasWebhookNodes, webhookNodes, hasPlayAudioNodes, playAudioNodes } = await this.fetchAndCompileFlow(flowIdToSync, params.knowledgeBaseIds || currentAgent.knowledgeBaseIds);
...
- Only say a short acknowledgment if needed (e.g. "Let me check that for you.")`;
    ...
    if (userCustomPrompt) {
      ...
      basePrompt = `${strictWorkflowPrompt}${datePrompt}`;
