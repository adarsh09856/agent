 ...
 * Returns only user-marked required form fields.
 * contactName is intentionally NOT in required to allow partial submissions
 * when a caller hangs up without completing all fields.
 */
...
export function getSubmitFormWebhookTool(
  ...
  // IMPORTANT: Do NOT include `required: []` (empty array) — ElevenLabs rejects it,
  // causing tool registration failure which breaks the entire workflow node.
 ...
 * 
 * Uses a SYSTEM OVERRIDE preamble to ensure the LLM ignores the base agent
 * system prompt (e.g. Joy Villa's investment qualification script) and ONLY
 * follows the form collection instructions for this specific node.
 */
