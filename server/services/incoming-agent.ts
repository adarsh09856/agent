...
export interface IncomingAgentUpdateParams {
  ...
  messagingWhatsappVariables?: Record<string, any>;
  expressiveMode?: boolean;
  voiceStability?: number;
  voiceSimilarityBoost?: number;
  voiceSpeed?: number;
...
export class IncomingAgentService {
  ...
  static async updateInElevenLabs(
    ...
    // Always include system prompt (resolved from params or fallback to currentAgent) to ensure it gets enhanced with tool instructions
    elevenLabsUpdates.prompt = params.systemPrompt ?? currentAgent.systemPrompt;
    ...
    elevenLabsUpdates.maxDurationSeconds = params.maxDurationSeconds !== undefined
      ? params.maxDurationSeconds
      : currentAgent.maxDurationSeconds;
    ...
    // Handle knowledge bases - use effective list of knowledge bases
    const effectiveKnowledgeBaseIds = params.knowledgeBaseIds !== undefined ? params.knowledgeBaseIds : currentAgent.knowledgeBaseIds;
    let hasKnowledgeBases = false;
    if (effectiveKnowledgeBaseIds !== undefined && effectiveKnowledgeBaseIds !== null) {
      console.log(`📚 [Incoming] Syncing ${effectiveKnowledgeBaseIds.length} knowledge base(s)`);
      const knowledgeBases = await this.fetchKnowledgeBases(effectiveKnowledgeBaseIds);
      elevenLabsUpdates.knowledge_bases = knowledgeBases;
      console.log(`   Total KB docs to sync: ${knowledgeBases.length}`);
      hasKnowledgeBases = effectiveKnowledgeBaseIds.length > 0;
    ...
    // Setup RAG workspace tool and appointment booking tool after agent update (separate API call for proper tool creation)
    ...
    console.log(`📚 [Incoming] Passing ${systemTools.length} system tools to RAG/appointment setup`);
