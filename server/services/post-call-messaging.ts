...
async function lookupAppointmentData(
  callId: string, 
  ...
  try {
    ...
    if (ids.length > 0 && userId) {
      ...
      if (rows.length > 0) {
        const a = rows[0];
        console.log(`[Post-Call Messaging] Found appointment data by callId/conversationId: date=${a.appointment_date}, time=${a.appointment_time}, service=${a.service_name || 'N/A'}`);
        return {
    ...
    // Fallback: search by phone number within the last 30 minutes
    if (callerPhone && userId) {
      ...
      if (digits.length >= 6) {
        ...
        const result = await db.execute(sql`
          ...
          WHERE user_id = ${userId}
            ...
            AND RIGHT(REGEXP_REPLACE(contact_phone, '[^0-9]', '', 'g'), 10) = ${last10}
          ORDER BY created_at DESC LIMIT 1
        `);
        const rows = extractRows(result);
        if (rows.length > 0) {
          const a = rows[0];
          console.log(`[Post-Call Messaging] Found appointment data by phone fallback: date=${a.appointment_date}, time=${a.appointment_time}, service=${a.service_name || 'N/A'}`);
          return {
            ...(a.contact_name ? { contact_name: a.contact_name } : {}),
            ...(a.contact_phone ? { contact_phone: a.contact_phone } : {}),
            ...(a.contact_email ? { contact_email: a.contact_email } : {}),
...
function mapLabelToKey(label: string): string | null {
  ...
  if (['customername', 'name', 'clientname', 'fullname', 'contactname', 'callername', 'username', 'patientname'].includes(norm)) {
    return 'contact_name';
  }
  if (['appointmentdateandtime', 'dateofappointmenttime', 'dateofappointmentandtime', 'appointmentdatetime', 'datetime', 'dateandtime', 'meetingdateandtime', 'appointmentdateamptime'].includes(norm)) {
    return 'appointment_date_time';
  }
  if (['appointmentdate', 'dateofappointment', 'date', 'meetingdate'].includes(norm)) {
    return 'appointment_date';
  }
  if (['appointmenttime', 'timeofappointment', 'time', 'meetingtime'].includes(norm)) {
    return 'appointment_time';
  }
  if (['service', 'servicename', 'serviceinterestedin', 'treatment', 'treatmentname'].includes(norm)) {
    return 'service_name';
  }
  if (['doctorname', 'doctor', 'agentname', 'agent', 'operator', 'clinic', 'clinicname', 'provider', 'providername', 'dentist', 'dentistname'].includes(norm)) {
    return 'agent_name';
  }
  if (['duration', 'meetingduration', 'length', 'calllength'].includes(norm)) {
    return 'duration';
  }
  if (['notes', 'additionalnotes', 'comment', 'comments', 'remarks', 'reason', 'reasonforvisit'].includes(norm)) {
    return 'notes';
  }
  return null;
}
...
function resolveVarValue(value: string, contactData: Record<string, string>): string {
  ...
  // Try fuzzy matching on human-friendly description labels
  const mappedKey = mapLabelToKey(trimmed);
  if (mappedKey) {
    if (mappedKey === 'appointment_date_time') {
      const date = contactData['appointment_date'] || '';
      const time = contactData['appointment_time'] || '';
      if (date && time) return `${date} at ${time}`;
      if (date) return date;
      if (time) return time;
    } else if (contactData[mappedKey] !== undefined && contactData[mappedKey] !== '') {
...
export async function triggerPostCallMessaging(params: {
  ...
  try {
    ...
    if (callId || conversationId) {
      const apptData = await lookupAppointmentData(callId || '', conversationId, userId, callerPhone);
            ...
            try {
              ...
              if (templateDef?.components) {
                ...
                if (bodyComp && bodyComp.text) {
                  ...
                  const requiredCount = Array.from(new Set(bodyVarMatches)).length;
                  if (requiredCount > 0) {
