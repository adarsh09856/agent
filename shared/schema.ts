...
export const agents = pgTable("agents", {
  ...
  telephonyProvider: text("telephony_provider").default("twilio"), // 'twilio' | 'plivo' | 'twilio_openai' | 'elevenlabs-sip' | 'openai-sip' | 'custom-voice-engine'
...
export const incomingConnections = pgTable("incoming_connections", {
  id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
  userId: varchar("user_id").notNull().references(() => users.id, { onDelete: "cascade" }),
  agentId: varchar("agent_id").notNull(), // Supports standard agents (agents.id) or Custom Voice Engine agents (ve_voice_agents.id)
...
// ============================================================
// Master AI Engine Tables
// ============================================================
export const veAgentInstantFaqs = pgTable("ve_agent_instant_faqs", {
  id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
...
export const insertVeAgentInstantFaqSchema = createInsertSchema(veAgentInstantFaqs).omit({
  id: true,
  createdAt: true,
  updatedAt: true,
});
export type InsertVeAgentInstantFaq = z.infer<typeof insertVeAgentInstantFaqSchema>;
export type VeAgentInstantFaq = typeof veAgentInstantFaqs.$inferSelect;

export const veMasterAiGlobalSettings = pgTable("ve_master_ai_global_settings", {
  id: serial("id").primaryKey(),
...
export const insertVeMasterAiGlobalSettingsSchema = createInsertSchema(veMasterAiGlobalSettings).omit({
  id: true,
  updatedAt: true,
});
export type InsertVeMasterAiGlobalSettings = z.infer<typeof insertVeMasterAiGlobalSettingsSchema>;
export type VeMasterAiGlobalSettings = typeof veMasterAiGlobalSettings.$inferSelect;
